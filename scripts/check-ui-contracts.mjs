  ...
  for (const match of source.matchAll(/className\s*=\s*"([^"]+)"/g)) {
    for (const className of match[1].trim().split(/\s+/)) {
      ...
      const selector = new RegExp(
        `\\.${className.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(?![-_a-zA-Z0-9])`,
      );
...
const actionBlock =
  guidance.match(
    /export const GLOBAL_ACTION_REGISTRY = \{([\s\S]*?)\n\} as const;/,
  )?.[1] ?? "";
const actionEntries = [
  ...actionBlock.matchAll(/^\s*"([^"]+)":\s*\{([\s\S]*?)^\s*\},?$/gm),
];
...
if (missingClasses.length || incompleteActions.length) {
  ...
  if (incompleteActions.length) {
    console.error(
      "Globale Aktionen ohne Label, Concept oder Hilfe:\n" +
...
console.log(
  `UI-Verträge gültig: statische Klassen abgedeckt, ${actionEntries.length} globale Aktionen dokumentiert.`,
);
