...
export function assessMeasurementTrust(
  ...
  if (measurement.deviceId) {
    ...
    if (calStatus === "FAILED" || calStatus === "INVALID_TIMESTAMP") {
      return blocked(
        "suspect",
        "Sensorkalibrierung ist fehlgeschlagen oder ungültig.",
      );
    }
    ...
    if (calStatus === "CALIBRATION_DUE") {
      reasons.push(
        "Kalibrierungsintervall ist abgelaufen. Messwert mit reduzierter Sicherheit.",
      );
    }
  ...
  if (reasons.length > 0) {
    const isOnlyCalibrationWarning = reasons.every((r) =>
      r.includes("Kalibrierung"),
    );
