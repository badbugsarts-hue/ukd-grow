...
describe("M1 Challenger Empirical Stress Harness", () => {
  describe("1. calculatePpfdMapSummary", () => {
    ...
    it("handles dimmer percent edge cases (0, >100, negative, NaN, Infinity)", () => {
      ...
      expect(
        calculatePpfdMapSummary(points, 40, Number.POSITIVE_INFINITY),
      ).toEqual({
        mean: 750,
        min: 500,
        max: 1000,
    ...
    it("property test: min <= mean and uniformity <= 1.0 across 1,000 random grids", () => {
      for (let i = 0; i < 1000; i++) {
        ...
        const points: PpfdMapPoint[] = Array.from(
          { length: count },
          (_, idx) => ({
            x: idx % 3,
            y: Math.floor(idx / 3),
  ...
  describe("4. getSensorCalibrationStatus", () => {
    ...
    it("enforces 30-day window for pH metric", () => {
      ...
      expect(
        getSensorCalibrationStatus("dev-1", "water.ph", [cal25DaysOld], now),
      ).toBe("DUE_SOON");
      ...
      expect(
        getSensorCalibrationStatus("dev-1", "water.ph", [cal35DaysOld], now),
    ...
    it("enforces 60-day window for EC metric", () => {
      ...
      expect(
        getSensorCalibrationStatus("dev-1", "water.ec", [cal45DaysOld], now),
      ).toBe("VALID");
      ...
      expect(
        getSensorCalibrationStatus("dev-1", "water.ec", [cal65DaysOld], now),
      ).toBe("CALIBRATION_DUE");
    });
