...
import { createRecoveryKit, createWorkspaceBackupV2, restoreWorkspaceKey, validateWorkspaceBackupV2 } from "./workspace-backup";
...
describe("workspace backup v2", () => {
	it("validates a manifested run roundtrip and rejects corruption", async () => {
		...
		const backup = await createWorkspaceBackupV2({ ...workspace, runIds: [run.id] }, [run]);
	...
	it("wraps and restores the workspace media key with a passphrase", async () => {
		...
		const restored = await restoreWorkspaceKey(kit, "correct horse battery staple");
		expect(await crypto.subtle.exportKey("raw", restored)).toEqual(await crypto.subtle.exportKey("raw", key));
		await expect(restoreWorkspaceKey(kit, "incorrect passphrase")).rejects.toThrow();
	}, 15_000);
});
