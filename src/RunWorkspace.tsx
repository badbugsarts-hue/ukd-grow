...
export function ReportsWorkspace({ run, onChange }: RunProps) {
	...
	const exportJson = async () =>
		download(
			...
			JSON.stringify(await createBackupEnvelope(run, "6.1.0"), null, 2),
			"application/json",
		);
	const exportCsv = () =>
		download(
	...
	const importRun = async (event: ChangeEvent<HTMLInputElement>) => {
		...
		if (!file) return;
		try {
			const parsed = await restoreBackup(
				JSON.parse(await file.text()) as unknown,
			);
			if (!parsed.ok) throw new Error(parsed.errors.join(" "));
			onChange({ ...parsed.value, updatedAt: new Date().toISOString() });
			setMessage(
				`✓ ${file.name} wurde per SHA-256 validiert${parsed.migrated ? ", migriert" : ""} und wiederhergestellt.`,
			);
		} catch (cause) {
			setMessage(
	...
	return (
		<div className="page-stack">
			...
			<section className="export-grid">
				...
				<ExportCard
					title="Restore"
					text="SHA-256-Envelope wird vor Restore validiert; v1/v2 werden kontrolliert nach v3 migriert."
...
export function SystemWorkspace({ run }: { run: RunPackage }) {
	...
	const exportDiagnostics = async () => {
		const snapshot = await createDiagnosticSnapshot(run, "6.1.0");
		download(
