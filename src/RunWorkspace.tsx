...
import { fieldGuidanceText, getFieldGuidance } from "./ui-guidance";
import { createBackupEnvelope, restoreBackup } from "./backup";
import { decryptWorkspaceCheckpoint } from "./backup-vault";
import platformQuality from "./data/platform-quality.json";
import { createDiagnosticSnapshot } from "./diagnostics";
...
import {
  ...
  saveWorkspaceMediaKey,
  stageAndActivateWorkspace,
  stageAndActivateRestoredRun,
} from "./run-storage";
...
const APPLICATION_VERSION = "8.0.0";

export function RunSetupWorkspace({ run, onChange }: RunProps) {
  const [draft, setDraft] = useState<RunConfig>(run.config);
  const [recommendationUndo, setRecommendationUndo] =
    useState<RunConfig | null>(null);
  ...
  const applyValidatedBaseline = () => {
    setRecommendationUndo(structuredClone(draft));
    setDraft((current) => ({
      ...current,
  ...
  return (
    <form className="page-stack" onSubmit={save}>
      ...
      >
        <header>
          <div>
            <small>BESTE NÄCHSTE OPTION · DETERMINISTISCH</small>
            <h2 id="setup-guidance-title">Geführtes Referenz-Setup</h2>
          </div>
        </header>
        <p>
          UKD kann den freigegebenen HESI-/Manual-Baseline-Preset übernehmen.
          Physische Maße, Genetik und unbekannte Wasserwerte bleiben
          unverändert. Bei {unknownWater} fehlenden Wasserangaben lautet die
          Empfehlung ausdrücklich: zuerst messen.
        </p>
        <div className="button-row">
          <button
            type="button"
            className="secondary-button"
            onClick={applyValidatedBaseline}
          >
          ...
          {recommendationUndo && (
            <button
              type="button"
              onClick={() => {
                setDraft(recommendationUndo);
                setRecommendationUndo(null);
              }}
            >
        ...
        <p className="form-note">
          Warum? Nur ein validierter Presetwert wird vorgeschlagen;
          Wasserchemie, Messwerte und Evidenz werden nie erfunden oder
          überschrieben.
        </p>
      </section>
...
export function ReportsWorkspace({ run, onChange }: RunProps) {
  ...
  const exportJson = async () =>
    download(
      ...
      JSON.stringify(
        await createBackupEnvelope(run, APPLICATION_VERSION),
        null,
        2,
      ),
      "application/json",
  ...
  const exportWorkspaceV2 = async () => {
    const runs = await indexedDbRunRepository.listRuns();
    const existingWorkspace = await loadWorkspacePackage();
    const workspace = {
      ...(existingWorkspace ?? createWorkspacePackage(config.name)),
      runIds: runs.map((entry) => entry.id),
    };
    const assets = runs.flatMap((entry) => entry.mediaAssets);
    const media = (
      await Promise.all(
        assets.map(async (asset) => {
          const ciphertext = await loadEncryptedMedia(asset.id);
          return ciphertext ? { assetId: asset.id, ciphertext } : null;
        }),
      )
    ).filter((entry) => entry !== null);
    download(
      `${safeName(config.name)}.ukd-workspace-v2.json`,
      JSON.stringify(
        await createWorkspaceBackupV2(workspace, runs, media),
        null,
        2,
      ),
      "application/json",
  ...
  const exportRecoveryKit = async () => {
    const passphrase = window.prompt(
      "Recovery-Passphrase (mindestens 12 Zeichen). Sicher und getrennt vom Backup aufbewahren:",
    );
    if (!passphrase) return;
    try {
      const kit = await createRecoveryKit(
        await getOrCreateWorkspaceMediaKey(),
        passphrase,
      );
      download(
        `${safeName(config.name)}.ukd-recovery-kit.json`,
        JSON.stringify(kit, null, 2),
        "application/json",
      );
      setMessage("✓ Recovery-Kit verschlüsselt exportiert.");
    } catch (cause) {
      setMessage(
        `Fehler: ${cause instanceof Error ? cause.message : "Recovery-Kit fehlgeschlagen."}`,
      );
    }
  };
  const importRun = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (
        raw &&
        typeof raw === "object" &&
        (raw as { format?: unknown }).format === "ukd-media-recovery-kit/1"
      ) {
        const passphrase = window.prompt(
          "Recovery-Passphrase eingeben. Sie wird nicht gespeichert:",
        );
        if (!passphrase) throw new Error("Recovery-Passphrase fehlt.");
        const key = await restoreWorkspaceKey(
          raw as Parameters<typeof restoreWorkspaceKey>[0],
          passphrase,
        );
        await saveWorkspaceMediaKey(key);
        setMessage(
          "✓ Recovery-Key geprüft und lokal installiert. Jetzt das verschlüsselte Backup importieren.",
        );
        return;
      }
      if (
        raw &&
        typeof raw === "object" &&
        (raw as { format?: unknown }).format ===
          "ukd-encrypted-workspace-checkpoint/1"
      ) {
        raw = await decryptWorkspaceCheckpoint(
          raw,
          await getOrCreateWorkspaceMediaKey(),
        );
      }
      if (
        raw &&
        typeof raw === "object" &&
        (raw as { format?: unknown }).format === "ukd-workspace-backup/2"
      ) {
        const workspaceRestore = await validateWorkspaceBackupV2(raw);
        if (!workspaceRestore.ok)
          throw new Error(workspaceRestore.errors.join(" "));
        const restored = await stageAndActivateWorkspace(
          workspaceRestore.workspace,
          workspaceRestore.runs,
          workspaceRestore.media,
        );
        onChange(restored);
        setMessage(
          `✓ ${file.name} wurde als Workspace-Backup-v2 manifestiert geprüft und atomar wiederhergestellt.`,
        );
        return;
      }
      const parsed = await restoreBackup(raw);
      if (!parsed.ok) throw new Error(parsed.errors.join(" "));
      const restored = await stageAndActivateRestoredRun({
        ...parsed.value,
        updatedAt: new Date().toISOString(),
      });
      onChange(restored);
      setMessage(
        `✓ ${file.name} wurde per SHA-256 validiert${parsed.migrated ? ", migriert" : ""}, isoliert geprüft und atomar wiederhergestellt.`,
      );
    } catch (cause) {
      setMessage(
  ...
  const exportXlsx = async () => {
    ...
    addSheet(
      ...
      run.incidents.map((entry) => ({
        ...entry,
        affectedEquipmentIds: JSON.stringify(entry.affectedEquipmentIds),
        affectedPlantIds: JSON.stringify(entry.affectedPlantIds),
    ...
    addSheet(
      ...
      run.postHarvest.map((entry) => ({
        ...entry,
        dryingCheckpoints: JSON.stringify(entry.dryingCheckpoints),
        cureCheckpoints: JSON.stringify(entry.cureCheckpoints),
      })),
    );
    addSheet(
      "Energy",
  ...
  return (
    <div className="page-stack">
      ...
      <section className="export-grid">
        <ExportCard
          title="Workspace Backup v2"
          text="Manifest, Hashliste, Run und optional verschlüsselte Medien als portables Archiv."
          action="Workspace-v2 sichern"
          onClick={() => void exportWorkspaceV2()}
        />
        <ExportCard
          title="Media Recovery Kit"
          text="Workspace-Key mit einer separaten Recovery-Passphrase schützen."
          action="Recovery-Kit sichern"
          onClick={() => void exportRecoveryKit()}
        />
        <ExportCard
          title="JSON Backup"
        ...
        <ExportCard
          title="Restore"
          text="SHA-256-Envelope wird vor Restore validiert; unterstützte Altschemata werden kontrolliert nach v4 migriert."
      ...
      <section className="panel">
        ...
        <ol className="backup-list">
          ...
          <li>SHA-256-Prüfsumme vor jeder Wiederherstellung validieren.</li>
          <li>
            Restore nach Schema- oder Versionswechsel testweise ausführen.
          </li>
...
export function SystemWorkspace({ run }: { run: RunPackage }) {
  ...
  const exportDiagnostics = async () => {
    ...
    download(
      `ukd-diagnostics-${new Date().toISOString().slice(0, 10)}.json`,
  ...
  return (
    <div className="page-stack">
      <section className="system-status-grid">
        <SystemCard
          label="Release"
          value={manifest?.applicationRelease ?? "lädt…"}
          state="target"
        />
        <SystemCard
          label="Netzwerk"
        ...
        <SystemCard
          label="Offline Cache"
          value={navigator.serviceWorker?.controller ? "AKTIV" : "NICHT AKTIV"}
          state={navigator.serviceWorker?.controller ? "measured" : "missing"}
        />
        <SystemCard
        ...
        <SystemCard
          label="Run-Speicher"
          value={run.storageState.state.toUpperCase()}
          state={run.storageState.state === "healthy" ? "measured" : "missing"}
        />
      </section>
      {run.storageState.state !== "healthy" && (
        <section className="panel inline-error" role="alert">
          <h2>Speicherschutz aktiv</h2>
          <p>
            {run.storageState.detail ??
              "Lokale Schreibzugriffe sind bis zur Wiederherstellung gesperrt."}
          </p>
          <p>
            Ungesicherte Änderungen:{" "}
            {run.storageState.unsavedChanges ? "JA" : "NEIN"}
          </p>
        </section>
      )}
      ...
      <section className="system-status-grid" aria-label="Platform-Qualität">
        <SystemCard
          label="Hazards"
          value={String(platformQuality.hazards.length)}
          state="measured"
        />
        <SystemCard
          label="Release-SLOs"
          value={String(
            platformQuality.slos.filter((entry) => entry.releaseGate).length,
          )}
          state="target"
        />
        <SystemCard
          label="Recovery-Pfade"
          value={String(platformQuality.recovery.testedPaths.length)}
          state="measured"
        />
        <SystemCard
          label="Aktor-Risiko"
          value="NICHT AKZEPTIERT"
          state="missing"
        />
      </section>
      <section className="panel integrity-panel">
        <header>
          <div>
            <small>LOCAL ONLY · PRIVACY PRESERVING</small>
            <h2>Support-Diagnose</h2>
          </div>
          <button
            className="secondary-button"
            type="button"
        ...
        <p>
          Enthält App-, Schema-, Speicher- und Integritätszustand sowie reine
          Objektanzahlen. Messwerte, Freitext, Rechtsprofil und Zugangsdaten
          werden ausdrücklich nicht exportiert.
        </p>
      </section>
  ...
  return (
    <label>
      <span>
        {label}{" "}
        <TermTooltip
          term={getFieldGuidance(label).concept}
          customText={fieldGuidanceText(label)}
        >
          ⓘ
        </TermTooltip>
      </span>
  ...
  return (
    <label>
      <span>
        ...
        {unit ? ` · ${unit}` : ""}{" "}
        <TermTooltip
          term={getFieldGuidance(label, unit, min, max).concept}
          customText={fieldGuidanceText(label, unit, min, max)}
        >
          ⓘ
        </TermTooltip>
      </span>
