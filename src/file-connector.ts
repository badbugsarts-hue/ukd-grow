  ...
  probe.rows.forEach((row, index) => {
    ...
    if (
      ...
      findings.push({
        ...
        code: "missing-field",
        message:
          "Zeitstempel, Metrik, Wert, Einheit, Gerät und Quelle sind Pflichtfelder.",
      });
      return;
    }
    if (!Number.isFinite(Date.parse(timestamp))) {
      findings.push({
        ...
        message:
          "Zeitstempel ist ungültig oder ohne interpretierbare Zeitzone.",
      });
      return;
    }
    if (!metrics.has(metricText as MeasurementMetric)) {
      findings.push({
        ...
        severity: "error",
        code: "unknown-metric",
        message: `Metrik ${metricText} ist nicht freigegeben.`,
      });
      return;
    }
    ...
    if (value === null) {
      findings.push({
        ...
        severity: "error",
        code: "invalid-number",
        message: `Wert ${rawValue} ist nicht eindeutig numerisch.`,
      });
      return;
    }
    ...
    if (allowedUnits && !allowedUnits.includes(unit)) {
      findings.push({
        ...
        severity: "error",
        code: "unit-mismatch",
        message: `${metric} benötigt ${allowedUnits.join(" oder ")}, nicht ${unit}.`,
      });
      return;
    }
    ...
    if (rowKeys.has(key)) {
      findings.push({
        ...
        severity: "error",
        code: "duplicate-row",
        message: "Messung ist innerhalb der Datei doppelt.",
      });
      return;
    }
    rowKeys.add(key);
...
export function parseLocaleNumber(value: string): number | null {
  ...
  if (comma >= 0 && dot >= 0) {
    ...
    const parsed = Number(
      normalized.replace(thousands, "").replace(decimal, "."),
    );
