...
function oracleFilter(
  ...
  const result = dataset.filter((s) => {
    ...
    if (levels.size > 0 && !levels.has(s.level as ExperienceLevel))
      return false;
...
describe("Empirical Challenger 2 — Autoflower Cockpit Adversarial Stress Suite", () => {
  ...
  describe("1. Cultivar Dataset Invariant & Range Verification (61 Strains)", () => {
    ...
    it("validates all 61 cultivars have unique IDs and unique ranks per category", () => {
      ...
      for (const strain of cockpitData) {
        expect(
          idSet.has(strain.id),
          `Duplicate ID detected: ${strain.id}`,
        ).toBe(false);
        idSet.add(strain.id);
      ...
      const jpRanks = cockpitData
        .filter((s) => s.kind === "jungpflanze")
        .map((s) => s.rank)
        .sort((a, b) => a - b);
      expect(jpRanks).toEqual(Array.from({ length: 50 }, (_, i) => i + 1));

      // Saatgut ranks 1..11
      const samenRanks = cockpitData
        .filter((s) => s.kind === "samen")
        .map((s) => s.rank)
        .sort((a, b) => a - b);
    ...
    it("verifies that required string fields are non-empty and non-whitespace", () => {
      ...
      for (const strain of cockpitData) {
        for (const field of stringFields) {
          ...
          expect(
            typeof val,
            `Strain ${strain.id} field ${String(field)} is not string`,
          ).toBe("string");
          expect(
    ...
    it("verifies positive yield ranges: ertrag_lo > 0 and ertrag_lo <= ertrag_hi", () => {
      for (const strain of cockpitData) {
        expect(
          Number.isFinite(strain.ertrag_lo),
          `Strain ${strain.id} ertrag_lo is not finite`,
        ).toBe(true);
        expect(
          Number.isFinite(strain.ertrag_hi),
          `Strain ${strain.id} ertrag_hi is not finite`,
        ).toBe(true);
        expect(
          strain.ertrag_lo,
          `Strain ${strain.id} ertrag_lo must be > 0`,
        ).toBeGreaterThan(0);
        expect(
          strain.ertrag_hi,
          `Strain ${strain.id} ertrag_hi must be >= ertrag_lo`,
        ).toBeGreaterThanOrEqual(strain.ertrag_lo);
      }
    });
    ...
    it("verifies positive height ranges: when present hmin > 0, hmax > 0, and hmin <= hmax", () => {
      ...
      for (const strain of cockpitData) {
        if (strain.hmin !== null && strain.hmax !== null) {
          ...
          expect(
            strain.hmin,
            `Strain ${strain.id} hmin must be > 0`,
          ).toBeGreaterThan(0);
          expect(
            strain.hmax,
            `Strain ${strain.id} hmax must be > 0`,
          ).toBeGreaterThan(0);
          expect(
            strain.hmin,
    ...
    it("verifies THC extraction yields finite numbers >= 0 across all strains", () => {
      for (const strain of cockpitData) {
        ...
        expect(
          Number.isFinite(numThc),
          `Strain ${strain.id} THC extraction is NaN`,
        ).toBe(true);
        expect(
          numThc,
          `Strain ${strain.id} THC extraction must be >= 0`,
        ).toBeGreaterThanOrEqual(0);
        expect(
          numThc,
          `Strain ${strain.id} THC extraction cannot exceed 45%`,
    ...
    it("verifies botanical enums and lineage consistency", () => {
      ...
      for (const s of cockpitData) {
        expect(validProv.has(s.prov), `Invalid prov ${s.prov} in ${s.id}`).toBe(
          true,
        );
        expect(validKind.has(s.kind), `Invalid kind ${s.kind} in ${s.id}`).toBe(
          true,
        );
        expect(validTyp.has(s.typ), `Invalid typ ${s.typ} in ${s.id}`).toBe(
          true,
        );
  ...
  describe("2. Combinatorial Filter Engine & Edge Case Robustness", () => {
    it("filters accurately across individual facets (breeder, mold, feed, height, level)", () => {
      ...
      expect(moldGood.every((s) => s.mold.toLowerCase().includes("gut"))).toBe(
        true,
      );
      ...
      expect(feedHigh.every((s) => s.feed.toLowerCase().includes("hoch"))).toBe(
        true,
      );
      ...
      expect(shortCanopy.every((s) => s.hmax !== null && s.hmax <= 80)).toBe(
        true,
      );
    });
    ...
    it("verifies panel UI rendering with initial data in SSR", () => {
      const html = renderToString(
        <AutoflowerCockpitPanel lens="guided" onSelectStrain={vi.fn()} />,
      );
    ...
    it("fuzz tests 300 random combinatorial filter queries ensuring determinism and stability", () => {
      ...
      const shops = [
        "",
        "Bushplanet",
        "Sensi Seeds direkt / Bushplanet",
        "BubatzBuddy",
        "Hanfbaron",
      ];
      const kinds: Array<"all" | CultivarKind> = [
        "all",
        "jungpflanze",
        "samen",
      ];
      ...
      for (let i = 0; i < 300; i++) {
        const criteria: FilterCriteria = {
          ...
          mold:
            i % 2 === 0
              ? new Set([moldOptions[i % moldOptions.length]])
              : new Set(),
          feed:
            i % 3 === 0
              ? new Set([feedOptions[i % feedOptions.length]])
              : new Set(),
          levels:
            i % 4 === 0 ? new Set([levels[i % levels.length]]) : new Set(),
  ...
  describe("3. Yield Uncertainty Calculation Boundaries & UI Math", () => {
    ...
    it("verifies yield uncertainty percentages across all 61 cultivars strictly adhere to bounds", () => {
      for (const strain of cockpitData) {
        const { leftPercent, widthPercent } = computeYieldPercentages(
          strain.ertrag_lo,
          strain.ertrag_hi,
        );
        ...
        expect(
          ...
          `Strain ${strain.id} leftPercent is NaN/infinite`,
        ).toBe(true);
        expect(
          Number.isFinite(widthPercent),
          `Strain ${strain.id} widthPercent is NaN/infinite`,
        ).toBe(true);
    ...
    it("verifies the 140W LED photobiology model: E_gesamt = 140W * [0.45 - 0.90 g/W] * q", () => {
      for (const strain of cockpitData) {
        ...
        expect(strain.ertrag_lo).toBeGreaterThanOrEqual(
          Math.floor(theoreticalMin) - 15,
        );
        expect(strain.ertrag_hi).toBeLessThanOrEqual(
          Math.ceil(theoreticalMax) + 15,
        );
      }
    });
  ...
  describe("4. Rapid Strain Selection, Modal Ergonomics & Keyboard Handlers", () => {
    ...
    it("simulates rapid consecutive strain selections and ensures callback integrity", () => {
      ...
      expect(onSelectStrain.mock.calls[60][0].name).toBe(
        "Banana Purple Punch Auto RF3",
      );
    });
      ...
      for (const opt of sortOptions) {
        ...
        if (opt === "score") {
          ...
          for (let i = 0; i < sorted.length - 1; i++) {
            ...
            const avgB =
              (sorted[i + 1].ertrag_lo + sorted[i + 1].ertrag_hi) / 2;
            expect(avgA).toBeGreaterThanOrEqual(avgB);
          }
        } else if (opt === "name") {
          for (let i = 0; i < sorted.length - 1; i++) {
            expect(
              sorted[i].name.localeCompare(sorted[i + 1].name, "de"),
