...
export async function validateWorkspaceBackupV2(
  value: unknown,
): Promise<
  | {
      ok: true;
...
> {
  if (!isBackup(value))
    return { ok: false, errors: ["Workspace-Backup-v2-Format fehlt."] };
  const errors: string[] = [];
  const manifestPaths = new Set(value.manifest.map((entry) => entry.path));
  if (manifestPaths.size !== value.manifest.length)
    errors.push("Manifest enthält doppelte Pfade.");
  for (const entry of value.manifest) {
    const encoded = value.files[entry.path];
    if (!encoded) {
      errors.push(`${entry.path}: Datei fehlt.`);
      continue;
    }
    const bytes = base64ToBytes(encoded);
    if (bytes.byteLength !== entry.byteLength)
      errors.push(`${entry.path}: Dateigröße stimmt nicht.`);
    if ((await sha256(bytes.buffer)) !== entry.sha256)
      errors.push(`${entry.path}: SHA-256 stimmt nicht.`);
  }
  for (const path of Object.keys(value.files))
    if (!manifestPaths.has(path))
      errors.push(`${path}: Datei ist nicht manifestiert.`);
  ...
  if (
    workspace.format !== "ukd-workspace-package" ||
    workspace.id !== value.workspaceId
  )
    return {
      ok: false,
      errors: ["Workspace-Identität oder Schema ist ungültig."],
    };
  const runs: RunPackage[] = [];
  for (const entry of value.manifest.filter((item) => item.kind === "run")) {
    try {
      const parsed = validateRunPackage(
        JSON.parse(base64ToUtf8(value.files[entry.path] ?? "")),
      );
      if (!parsed.ok) errors.push(`${entry.path}: ${parsed.errors.join(" ")}`);
      else runs.push(parsed.value);
    } catch {
      errors.push(`${entry.path}: Run ist nicht lesbar.`);
    }
  }
  const media = value.manifest
    .filter((entry) => entry.kind === "media")
    .map((entry) => ({
      assetId: entry.path.slice(6, -4),
      ciphertext: base64ToBytes(value.files[entry.path] ?? "").buffer,
    }));
  const assetIds = new Set(
    runs.flatMap((run) => run.mediaAssets.map((asset) => asset.id)),
  );
  for (const entry of media)
    if (!assetIds.has(entry.assetId))
      errors.push(`media/${entry.assetId}: keine fachliche Referenz.`);
  return errors.length
    ? { ok: false, errors }
    : { ok: true, workspace, runs, media };
}
...
export async function createRecoveryKit(
  ...
  if (passphrase.length < 12)
    throw new Error("Recovery-Passphrase benötigt mindestens 12 Zeichen.");
...
export async function restoreWorkspaceKey(
  ...
  try {
    raw = await crypto.subtle.decrypt(
      { name: "AES-GCM", iv },
      wrappingKey,
      base64ToBytes(kit.wrappedKey),
    );
  } catch {
    throw new Error("Recovery-Passphrase oder Recovery-Kit ist ungültig.");
  }
  if ((await sha256(raw)) !== kit.keySha256)
    throw new Error("Recovery-Key-Prüfsumme stimmt nicht.");
...
function isBackup(value: unknown): value is WorkspaceBackupV2 {
  return (
    Boolean(value) &&
    typeof value === "object" &&
    (value as WorkspaceBackupV2).format === "ukd-workspace-backup/2" &&
    Array.isArray((value as WorkspaceBackupV2).manifest) &&
    Boolean((value as WorkspaceBackupV2).files)
  );
}
