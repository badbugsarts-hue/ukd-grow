import { DAILY_COLUMNS, type DayPlan, numberAt } from "./domain";
import type {
  AuditEvent,
  DailyObservation,
  DayZeroAnchor,
  DomainEvent,
  DomainEventType,
  GrowthEvent,
  InventoryEvent,
  LiveAnchor,
  LiveAnchorRevision,
  Measurement,
  MeasurementMetric,
  ObservationValues,
  PlantIdentity,
  PlantMilestones,
  PotProfile,
  RunAlert,
  RunConfig,
  RunEvent,
  RunExecutionMode,
  RunOverride,
  RunPackage,
  RunTask,
  StructuredObservation,
  WaterProfile,
} from "./types";

export const RUN_SCHEMA_VERSION = "6.0.0" as const;

const measurementMap: Array<{
  key: keyof ObservationValues;
  metric: MeasurementMetric;
  unit?: Measurement["reading"]["unit"];
}> = [
  { key: "tempMax", metric: "temperature.air.max", unit: "°C" },
  { key: "tempMin", metric: "temperature.air.min", unit: "°C" },
  { key: "humidityMax", metric: "humidity.relative.max", unit: "%" },
  { key: "humidityMin", metric: "humidity.relative.min", unit: "%" },
  { key: "leafTemp", metric: "temperature.leaf", unit: "°C" },
  { key: "ppfd", metric: "light.ppfd", unit: "µmol/m²/s" },
  { key: "phIn", metric: "water.ph", unit: "pH" },
  { key: "ecIn", metric: "water.ec", unit: "mS/cm" },
  { key: "phDrain", metric: "drain.ph", unit: "pH" },
  { key: "ecDrain", metric: "drain.ec", unit: "mS/cm" },
  { key: "waterLiters", metric: "water.volume", unit: "L" },
  { key: "drainLiters", metric: "drain.volume", unit: "L" },
  { key: "potMassGrams", metric: "pot.mass", unit: "g" },
  { key: "plantHeightCm", metric: "plant.height", unit: "cm" },
  { key: "stress", metric: "plant.stress" },
];

const emptyValues = (): ObservationValues => ({
  tempMax: null,
  tempMin: null,
  humidityMax: null,
  humidityMin: null,
  leafTemp: null,
  ppfd: null,
  phIn: null,
  ecIn: null,
  phDrain: null,
  ecDrain: null,
  waterLiters: null,
  drainLiters: null,
  drainPercent: null,
  potMassGrams: null,
  plantHeightCm: null,
  stress: null,
});

export function createDefaultRunPackage(now = new Date()): RunPackage {
  const timestamp = now.toISOString();
  const runId = crypto.randomUUID();
  const zoneId = crypto.randomUUID();
  const plantId = crypto.randomUUID();
  const config: RunConfig = {
    name: "UKD Masterplan v11 Variante B (LST)",
    genetics: "Double Grape",
    startDate: timestamp.slice(0, 10),
    endDay: 80,
    plantCount: 3,
    dayZeroAnchor: "run-operational-start",
    tentWidthCm: 60,
    tentDepthCm: 60,
    tentHeightCm: 180,
    ledMaxW: 140,
    lightHours: 18,
    medium: "Eazy Block · Pot9 Rhiza",
    mediumProduct: "Pot9 Rhiza",
    irrigationSystem: "Manuell / indoor-freigegeben",
    nutrientSystem: "UKD HESI Conservative",
    water: defaultWaterProfile(),
    pot: {
      type: "other",
      nominalVolumeLiters: 9,
      actualFillLiters: null,
      diameterCm: null,
      heightCm: null,
      emptyMassGrams: null,
      saturatedMassGrams: null,
    },
    light: null,
  };
  return {
    schemaVersion: RUN_SCHEMA_VERSION,
    id: runId,
    createdAt: timestamp,
    updatedAt: timestamp,
    status: "draft",
    config,
    configurationSnapshot: {
      id: crypto.randomUUID(),
      version: 1,
      capturedAt: timestamp,
      config: copyConfig(config),
      evidenceVersion: "v8 Evidence-Guarded",
      immutable: true,
    },
    zones: [
      {
        id: zoneId,
        name: "Hauptzone",
        kind: "tent",
        widthCm: config.tentWidthCm,
        depthCm: config.tentDepthCm,
        heightCm: config.tentHeightCm,
      },
    ],
    plants: [
      {
        id: plantId,
        zoneId,
        label: "Pflanze A1",
        genetics: config.genetics,
        status: "planned",
        identity: defaultPlantIdentity(),
      },
    ],
    devices: [],
    calibrations: [],
    observations: [],
    measurements: [],
    structuredObservations: [],
    tasks: [],
    overrides: [],
    auditEvents: [
      auditEvent(
        "run-created",
        "run",
        runId,
        "Run als Entwurf angelegt.",
        timestamp,
      ),
    ],
    domainEvents: [
      domainEvent(
        runId,
        "run.created",
        { configurationSnapshotId: "draft" },
        timestamp,
      ),
    ],
    events: [],
    completedTasks: {},
    acknowledgedAlertIds: [],
    inventory: [],
    privacyClassification: "project",
    growthEvents: [],
    irrigationEvents: [],
    mixBatches: [],
    reservoirs: [],
    equipment: [],
    maintenanceEvents: [],
    lungRoom: null,
    ipmInspections: [],
    incidents: [],
    postHarvest: [],
    energyReadings: [],
    cultivarProfiles: [],
    productInventory: [],
    nutrientSystems: [],
    mixApplications: [],
    mediaAssets: [],
    storageState: {
      state: "healthy",
      lastSuccessfulWriteAt: timestamp,
      lastFailureAt: null,
      failureKind: null,
      detail: null,
      unsavedChanges: false,
    },
    executionMode: "simulation",
    sourceSimulationRunId: null,
    liveAnchor: null,
    anchorRevisions: [],
    clockHealth: {
      status: "healthy",
      lastObservedAtUtc: timestamp,
      detail: null,
    },
    backupState: {
      pending: false,
      lastCheckpointAtUtc: null,
      lastCheckpointSha256: null,
      lastVerifiedAtUtc: null,
      lastExternalBackupAtUtc: null,
      externalMode: "none",
      persistentStorage: "unknown",
      usageBytes: null,
      quotaBytes: null,
    },
    aiProposalReviews: [],
    backupCheckpoints: [],
  };
}

export function createObservation(
  day: number,
  now = new Date(),
): DailyObservation {
  return {
    id: crypto.randomUUID(),
    day,
    recordedAt: now.toISOString(),
    source: "manual",
    status: "measured",
    values: emptyValues(),
    notes: "",
  };
}

export function latestObservation(
  run: RunPackage,
  day: number,
): DailyObservation | undefined {
  return run.observations
    .filter((entry) => entry.day === day)
    .sort((a, b) => b.recordedAt.localeCompare(a.recordedAt))[0];
}

export function addObservation(
  run: RunPackage,
  observation: DailyObservation,
): RunPackage {
  const measurements = measurementsFromObservation(run, observation);
  const event: RunEvent = {
    id: crypto.randomUUID(),
    day: observation.day,
    occurredAt: observation.recordedAt,
    category: "measurement",
    title: "Messdatensatz gespeichert",
    detail: observation.notes || "Manuelle Messwerte erfasst",
    relatedEntityId: observation.id,
  };
  return touch({
    ...run,
    observations: [...run.observations, observation],
    measurements: [...run.measurements, ...measurements],
    auditEvents: [
      auditEvent(
        "measurement-recorded",
        "observation",
        observation.id,
        `${measurements.length} typisierte Messwerte erfasst.`,
        observation.recordedAt,
      ),
      ...run.auditEvents,
    ],
    domainEvents: [
      domainEvent(
        run.id,
        "measurement.recorded",
        {
          observationId: observation.id,
          measurementIds: measurements.map((entry) => entry.id),
        },
        observation.recordedAt,
      ),
      ...run.domainEvents,
    ],
    events: [event, ...run.events],
  });
}

export function addRunEvent(run: RunPackage, event: RunEvent): RunPackage {
  return touch({
    ...run,
    events: [event, ...run.events],
    auditEvents: [
      auditEvent(
        "observation-recorded",
        "run-event",
        event.id,
        `${event.title}: ${event.detail}`,
        event.occurredAt,
      ),
      ...run.auditEvents,
    ],
    domainEvents: [
      domainEvent(
        run.id,
        "observation.recorded",
        { eventId: event.id, category: event.category },
        event.occurredAt,
      ),
      ...run.domainEvents,
    ],
  });
}

export function addInventoryEvent(
  run: RunPackage,
  event: InventoryEvent,
): RunPackage {
  return touch({
    ...run,
    inventory: [event, ...run.inventory],
    domainEvents: [
      domainEvent(
        run.id,
        "inventory.recorded",
        { inventoryEventId: event.id, type: event.type },
        event.occurredAt,
      ),
      ...run.domainEvents,
    ],
  });
}

export function updateRunConfig(
  run: RunPackage,
  config: RunConfig,
): RunPackage {
  const occurredAt = new Date().toISOString();
  return touch({
    ...run,
    config,
    ...(run.status === "draft" ? synchronizedAssets(run, config) : {}),
    events: [
      {
        id: crypto.randomUUID(),
        day: 0,
        occurredAt,
        category: "configuration-change",
        title:
          run.status === "draft"
            ? "Run-Entwurf aktualisiert"
            : "Setup-Profil aktualisiert; aktiver Snapshot bleibt unverändert",
        detail: `${config.genetics} · ${config.medium} · ${config.ledMaxW} W`,
      },
      ...run.events,
    ],
    auditEvents: [
      auditEvent(
        "configuration-changed",
        "run-config",
        run.id,
        run.status === "draft"
          ? "Entwurfskonfiguration geändert."
          : `Profil geändert; Snapshot ${run.configurationSnapshot.id} nicht verändert.`,
        occurredAt,
      ),
      ...run.auditEvents,
    ],
    domainEvents: [
      domainEvent(
        run.id,
        "configuration.changed",
        { activeSnapshotPreserved: run.status !== "draft" },
        occurredAt,
      ),
      ...run.domainEvents,
    ],
  });
}

export function updatePlantIdentity(
  run: RunPackage,
  genetics: string,
  identity: PlantIdentity,
  dayZeroAnchor: DayZeroAnchor,
  anchorDate: string,
): RunPackage {
  const occurredAt = new Date().toISOString();
  const primaryPlant = run.plants[0];
  const plantId = primaryPlant?.id ?? crypto.randomUUID();

  const updatedPlants = (
    run.plants.length > 0
      ? run.plants
      : [
          {
            id: plantId,
            zoneId: run.zones[0]?.id ?? "unassigned-zone",
            label: "Pflanze A1",
            genetics,
            status: "planned" as const,
            identity,
          },
        ]
  ).map((plant) => ({
    ...plant,
    genetics,
    identity: {
      ...plant.identity,
      ...identity,
    },
  }));

  const existingGrowthEvents = (run.growthEvents || []).filter(
    (e) => e.kind !== dayZeroAnchor,
  );

  const anchorOccurredAt = anchorDate
    ? new Date(`${anchorDate}T00:00:00Z`).toISOString()
    : occurredAt;

  const newAnchorEvent: GrowthEvent = {
    id: crypto.randomUUID(),
    plantId,
    kind: dayZeroAnchor,
    occurredAt: anchorOccurredAt,
    day: 0,
    observedBy: "user",
    confidence: "confirmed",
    notes: `Day Zero Anker gesetzt (${dayZeroAnchor})`,
    photoIds: [],
  };

  const updatedGrowthEvents = [newAnchorEvent, ...existingGrowthEvents];

  const updatedConfig: RunConfig = {
    ...run.config,
    genetics,
    dayZeroAnchor,
  };

  return touch({
    ...run,
    config: updatedConfig,
    plants: updatedPlants,
    growthEvents: updatedGrowthEvents,
    auditEvents: [
      auditEvent(
        "configuration-changed",
        "plant-identity",
        plantId,
        `Pflanzenidentität aktualisiert: Genetik '${genetics}', Breeder '${identity.breeder ?? "—"}', Day Zero Anker '${dayZeroAnchor}' am ${anchorDate}.`,
        occurredAt,
      ),
      ...run.auditEvents,
    ],
    domainEvents: [
      domainEvent(
        run.id,
        "configuration.changed",
        {
          plantId,
          genetics,
          breeder: identity.breeder,
          seedLot: identity.seedLot,
          dayZeroAnchor,
          anchorDate,
        },
        occurredAt,
      ),
      ...run.domainEvents,
    ],
  });
}

export function updateExecutionMode(
  run: RunPackage,
  mode: RunExecutionMode,
): RunPackage {
  if (run.executionMode === mode) {
    return run;
  }
  const occurredAt = new Date().toISOString();

  let liveAnchor = run.liveAnchor;
  let clockHealth = run.clockHealth;

  if (mode === "live") {
    if (!liveAnchor) {
      const anchorDate = run.config.startDate
        ? (run.config.startDate.includes("T")
            ? run.config.startDate
            : new Date(`${run.config.startDate}T00:00:00Z`).toISOString())
        : occurredAt;
      liveAnchor = {
        id: crypto.randomUUID(),
        kind: "seed-planted",
        startedAtUtc: anchorDate,
        confirmedAtUtc: occurredAt,
        timeZoneAtConfirmation:
          Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC",
      };
    }
    clockHealth = {
      status: "healthy",
      lastObservedAtUtc: occurredAt,
      detail: null,
    };
  }

  return touch({
    ...run,
    executionMode: mode,
    status: mode === "live" && run.status === "draft" ? "active" : run.status,
    liveAnchor,
    clockHealth,
    events: [
      {
        id: crypto.randomUUID(),
        day: 0,
        occurredAt,
        category: mode === "live" ? "live" : "configuration-change",
        title:
          mode === "live"
            ? "Live-Modus aktiviert"
            : "Simulations-Modus aktiviert",
        detail:
          mode === "live"
            ? "Run in den aktiven Live-Modus versetzt."
            : "Run in den Simulations-Modus versetzt.",
      },
      ...run.events,
    ],
    auditEvents: [
      auditEvent(
        mode === "live" ? "live-started" : "configuration-changed",
        "run-execution-mode",
        run.id,
        `Ausführungsmodus geändert von '${run.executionMode}' auf '${mode}'.`,
        occurredAt,
      ),
      ...run.auditEvents,
    ],
    domainEvents: [
      domainEvent(
        run.id,
        mode === "live" ? "live.started" : "configuration.changed",
        {
          previousMode: run.executionMode,
          nextMode: mode,
        },
        occurredAt,
      ),
      ...run.domainEvents,
    ],
  });
}

export function updatePlantMilestones(
  run: RunPackage,
  milestones: PlantMilestones,
  reason = "Pflanzen-Meilensteine aktualisiert",
): RunPackage {
  const occurredAt = new Date().toISOString();
  const primaryPlant = run.plants[0];
  const plantId = primaryPlant?.id ?? crypto.randomUUID();
  const dayZeroAnchor: DayZeroAnchor =
    milestones.dayZeroAnchor ?? run.config.dayZeroAnchor ?? "emergence";

  const normalizeIso = (dateStr?: string): string | null => {
    if (!dateStr) return null;
    const trimmed = dateStr.trim();
    if (!trimmed) return null;
    if (trimmed.includes("T")) {
      const parsed = new Date(trimmed);
      return Number.isNaN(parsed.getTime()) ? null : parsed.toISOString();
    }
    const parsed = new Date(`${trimmed}T00:00:00Z`);
    return Number.isNaN(parsed.getTime()) ? null : parsed.toISOString();
  };

  const pottingIso = normalizeIso(milestones.pottingDateIso);
  const emergenceIso = normalizeIso(milestones.emergenceDateIso);

  const otherGrowthEvents = (run.growthEvents || []).filter(
    (e) => e.kind !== "seed-planted" && e.kind !== "emergence",
  );

  const newGrowthEvents: GrowthEvent[] = [];

  if (pottingIso) {
    newGrowthEvents.push({
      id: crypto.randomUUID(),
      plantId,
      kind: "seed-planted",
      occurredAt: pottingIso,
      day: dayZeroAnchor === "seed-planted" ? 0 : null,
      observedBy: "user",
      confidence: "confirmed",
      notes: "Aussaat / Eintopfen (Meilenstein)",
      photoIds: [],
    });
  }

  if (emergenceIso) {
    newGrowthEvents.push({
      id: crypto.randomUUID(),
      plantId,
      kind: "emergence",
      occurredAt: emergenceIso,
      day: dayZeroAnchor === "emergence" ? 0 : null,
      observedBy: "user",
      confidence: "confirmed",
      notes: "Keimung / Durchstoß (Day Zero Meilenstein)",
      photoIds: [],
    });
  }

  const updatedGrowthEvents = [...newGrowthEvents, ...otherGrowthEvents];

  let effectiveAnchorDateIso: string | null = null;
  if (dayZeroAnchor === "emergence" && emergenceIso) {
    effectiveAnchorDateIso = emergenceIso;
  } else if (dayZeroAnchor === "seed-planted" && pottingIso) {
    effectiveAnchorDateIso = pottingIso;
  } else if (emergenceIso) {
    effectiveAnchorDateIso = emergenceIso;
  } else if (pottingIso) {
    effectiveAnchorDateIso = pottingIso;
  } else if (run.config.startDate) {
    effectiveAnchorDateIso = normalizeIso(run.config.startDate);
  }

  const effectiveAnchorDayStr = effectiveAnchorDateIso
    ? effectiveAnchorDateIso.slice(0, 10)
    : run.config.startDate;

  const updatedPlants = (
    run.plants.length > 0
      ? run.plants
      : [
          {
            id: plantId,
            zoneId: run.zones[0]?.id ?? "unassigned-zone",
            label: "Pflanze A1",
            genetics: run.config.genetics,
            status: "planned" as const,
            identity: defaultPlantIdentity(),
          },
        ]
  ).map((plant) => ({
    ...plant,
    identity: {
      ...plant.identity,
      pottingDateIso: milestones.pottingDateIso ?? plant.identity.pottingDateIso,
      emergenceDateIso:
        milestones.emergenceDateIso ?? plant.identity.emergenceDateIso,
      dayZeroAnchorDate: effectiveAnchorDayStr,
    },
  }));

  let updatedLiveAnchor = run.liveAnchor;
  const nextAnchorRevisions = [...run.anchorRevisions];

  if (run.liveAnchor && effectiveAnchorDateIso) {
    if (run.liveAnchor.startedAtUtc !== effectiveAnchorDateIso) {
      nextAnchorRevisions.unshift({
        id: crypto.randomUUID(),
        previousStartedAtUtc: run.liveAnchor.startedAtUtc,
        nextStartedAtUtc: effectiveAnchorDateIso,
        reason: reason.trim() || "Meilenstein-Korrektur (Retroaktiv)",
        correctedAtUtc: occurredAt,
      });
      updatedLiveAnchor = {
        ...run.liveAnchor,
        startedAtUtc: effectiveAnchorDateIso,
      };
    }
  }

  const updatedConfig: RunConfig = {
    ...run.config,
    startDate: effectiveAnchorDayStr,
    dayZeroAnchor,
  };

  return touch({
    ...run,
    config: updatedConfig,
    plants: updatedPlants,
    growthEvents: updatedGrowthEvents,
    liveAnchor: updatedLiveAnchor,
    anchorRevisions: nextAnchorRevisions,
    clockHealth: {
      status: "healthy",
      lastObservedAtUtc: occurredAt,
      detail: null,
    },
    events: [
      {
        id: crypto.randomUUID(),
        day: 0,
        occurredAt,
        category: "growth-event",
        title: "Pflanzen-Meilensteine aktualisiert",
        detail: `Eintopfen: ${milestones.pottingDateIso ?? "—"} · Keimung: ${milestones.emergenceDateIso ?? "—"} (Anker: ${dayZeroAnchor})`,
      },
      ...run.events,
    ],
    auditEvents: [
      auditEvent(
        "configuration-changed",
        "plant-milestone",
        plantId,
        `Meilensteine aktualisiert: Eintopfen ${milestones.pottingDateIso ?? "—"}, Keimung ${milestones.emergenceDateIso ?? "—"} (Anker: ${dayZeroAnchor}). ${reason}`.trim(),
        occurredAt,
      ),
      ...run.auditEvents,
    ],
    domainEvents: [
      domainEvent(
        run.id,
        "configuration.changed",
        {
          plantId,
          pottingDateIso: milestones.pottingDateIso,
          emergenceDateIso: milestones.emergenceDateIso,
          dayZeroAnchor,
          effectiveAnchorDayStr,
          reason,
        },
        occurredAt,
      ),
      ...run.domainEvents,
    ],
  });
}

export function updatePotProfile(run: RunPackage, pot: PotProfile): RunPackage {
  const occurredAt = new Date().toISOString();
  const updatedPot: PotProfile = {
    type: pot.type || run.config.pot.type || "other",
    nominalVolumeLiters: Math.max(0.1, pot.nominalVolumeLiters || 10),
    actualFillLiters:
      pot.actualFillLiters !== null && pot.actualFillLiters !== undefined
        ? Math.max(0.1, pot.actualFillLiters)
        : null,
    diameterCm:
      pot.diameterCm !== null && pot.diameterCm !== undefined
        ? pot.diameterCm
        : null,
    heightCm:
      pot.heightCm !== null && pot.heightCm !== undefined ? pot.heightCm : null,
    emptyMassGrams:
      pot.emptyMassGrams !== null && pot.emptyMassGrams !== undefined
        ? Math.max(0, pot.emptyMassGrams)
        : null,
    saturatedMassGrams:
      pot.saturatedMassGrams !== null && pot.saturatedMassGrams !== undefined
        ? Math.max(0, pot.saturatedMassGrams)
        : null,
  };

  const updatedConfig: RunConfig = {
    ...run.config,
    pot: updatedPot,
  };

  return touch({
    ...run,
    config: updatedConfig,
    ...(run.status === "draft" ? synchronizedAssets(run, updatedConfig) : {}),
    events: [
      {
        id: crypto.randomUUID(),
        day: 0,
        occurredAt,
        category: "configuration-change",
        title:
          run.status === "draft"
            ? "Topfprofil aktualisiert"
            : "Topfprofil aktualisiert; aktiver Snapshot bleibt unverändert",
        detail: `${updatedPot.nominalVolumeLiters}L (${updatedPot.type}) · Tara: ${updatedPot.emptyMassGrams ?? "-"}g · Sat: ${updatedPot.saturatedMassGrams ?? "-"}g`,
      },
      ...run.events,
    ],
    auditEvents: [
      auditEvent(
        "configuration-changed",
        "pot-profile",
        run.id,
        run.status === "draft"
          ? `Topfprofil konfiguriert: ${updatedPot.nominalVolumeLiters}L, Tara ${updatedPot.emptyMassGrams ?? 0}g, Sättigung ${updatedPot.saturatedMassGrams ?? 0}g.`
          : `Topfprofil geändert; Snapshot ${run.configurationSnapshot.id} nicht verändert.`,
        occurredAt,
      ),
      ...run.auditEvents,
    ],
    domainEvents: [
      domainEvent(
        run.id,
        "configuration.changed",
        {
          pot: updatedPot,
          activeSnapshotPreserved: run.status !== "draft",
        },
        occurredAt,
      ),
      ...run.domainEvents,
    ],
  });
}

export function activateRun(run: RunPackage, now = new Date()): RunPackage {
  if (run.status !== "draft") return run;
  const occurredAt = now.toISOString();
  const configurationSnapshot = {
    id: crypto.randomUUID(),
    version: run.configurationSnapshot.version + 1,
    capturedAt: occurredAt,
    config: copyConfig(run.config),
    evidenceVersion: "v8 Evidence-Guarded",
    immutable: true as const,
  };
  return touch({
    ...run,
    status: "active",
    configurationSnapshot,
    plants: run.plants.map((plant) => ({
      ...plant,
      status: "active" as const,
    })),
    auditEvents: [
      auditEvent(
        "run-activated",
        "configuration-snapshot",
        configurationSnapshot.id,
        `Run mit Snapshot v${configurationSnapshot.version} aktiviert.`,
        occurredAt,
      ),
      ...run.auditEvents,
    ],
    domainEvents: [
      domainEvent(
        run.id,
        "run.activated",
        { configurationSnapshotId: configurationSnapshot.id },
        occurredAt,
      ),
      ...run.domainEvents,
    ],
    events: [
      {
        id: crypto.randomUUID(),
        day: 0,
        occurredAt,
        category: "phase-change",
        title: "Run aktiviert",
        detail: `Immutable Konfiguration v${configurationSnapshot.version}`,
        relatedEntityId: configurationSnapshot.id,
      },
      ...run.events,
    ],
  });
}

export function effectiveRunConfig(run: RunPackage): RunConfig {
  return run.status === "draft" ? run.config : run.configurationSnapshot.config;
}

export function setTaskCompleted(
  run: RunPackage,
  day: number,
  task: string,
  completed: boolean,
): RunPackage {
  const key = String(day);
  const current = new Set(run.completedTasks[key] ?? []);
  if (completed) current.add(task);
  else current.delete(task);
  const taskId = taskKey(day, task);
  const occurredAt = new Date().toISOString();
  const existing = run.tasks.find((entry) => entry.id === taskId);
  const semanticTask: RunTask = {
    id: taskId,
    runId: run.id,
    day,
    type: existing?.type ?? "daily-operation",
    title: task,
    requirement: existing?.requirement ?? "required",
    dueAt: existing?.dueAt ?? null,
    phase: existing?.phase ?? "plan-derived",
    state: completed ? "completed" : "due",
    reason: existing?.reason ?? "Aus kanonischem Tagesplan abgeleitet.",
    evidenceRefs: existing?.evidenceRefs ?? [],
    dependencies: existing?.dependencies ?? [],
    ...(completed ? { completedAt: occurredAt } : {}),
  };
  return touch({
    ...run,
    completedTasks: { ...run.completedTasks, [key]: [...current] },
    tasks: [semanticTask, ...run.tasks.filter((entry) => entry.id !== taskId)],
    auditEvents: [
      auditEvent(
        "task-state-changed",
        "task",
        taskId,
        `Status auf ${semanticTask.state} gesetzt.`,
        occurredAt,
      ),
      ...run.auditEvents,
    ],
    domainEvents: [
      domainEvent(
        run.id,
        "task.transitioned",
        { taskId, from: existing?.state ?? "planned", to: semanticTask.state },
        occurredAt,
      ),
      ...run.domainEvents,
    ],
  });
}

const taskTransitions: Record<RunTask["state"], RunTask["state"][]> = {
  planned: ["due", "blocked", "skipped"],
  due: ["ready", "blocked", "skipped"],
  ready: ["completed", "blocked", "skipped"],
  completed: [],
  skipped: ["due"],
  blocked: ["due", "ready", "skipped"],
};

export function transitionTaskState(
  run: RunPackage,
  taskId: string,
  nextState: RunTask["state"],
  reason: string,
  now = new Date(),
): { ok: true; run: RunPackage } | { ok: false; error: string } {
  const task = run.tasks.find((entry) => entry.id === taskId);
  if (!task) return { ok: false, error: "Aufgabe wurde nicht gefunden." };
  if (!taskTransitions[task.state].includes(nextState)) {
    return {
      ok: false,
      error: `Transition ${task.state} → ${nextState} ist nicht zulässig.`,
    };
  }
  if ((nextState === "blocked" || nextState === "skipped") && !reason.trim()) {
    return {
      ok: false,
      error: "Blockieren oder Überspringen benötigt einen Grund.",
    };
  }
  const occurredAt = now.toISOString();
  const nextTask: RunTask = {
    ...task,
    state: nextState,
    reason: reason.trim() || task.reason,
    ...(nextState === "completed" ? { completedAt: occurredAt } : {}),
  };
  return {
    ok: true,
    run: touch({
      ...run,
      tasks: [nextTask, ...run.tasks.filter((entry) => entry.id !== taskId)],
      auditEvents: [
        auditEvent(
          "task-state-changed",
          "task",
          taskId,
          `${task.state} → ${nextState}: ${nextTask.reason}`,
          occurredAt,
        ),
        ...run.auditEvents,
      ],
      domainEvents: [
        domainEvent(
          run.id,
          "task.transitioned",
          { taskId, from: task.state, to: nextState, reason: nextTask.reason },
          occurredAt,
        ),
        ...run.domainEvents,
      ],
    }),
  };
}

export interface RunEventProjection {
  status: RunPackage["status"];
  measurementsRecorded: number;
  measurementsSuperseded: number;
  observationsRecorded: number;
  taskTransitions: number;
  activeOverrides: Set<string>;
  lastEventAt: string | null;
}

export function projectDomainEvents(events: DomainEvent[]): RunEventProjection {
  return [...events]
    .sort((left, right) => left.occurredAt.localeCompare(right.occurredAt))
    .reduce<RunEventProjection>(
      (projection, event) => {
        projection.lastEventAt = event.occurredAt;
        if (event.type === "run.activated") projection.status = "active";
        if (event.type === "measurement.recorded") {
          const ids = event.payload.measurementIds;
          projection.measurementsRecorded += Array.isArray(ids)
            ? ids.length
            : 1;
        }
        if (event.type === "measurement.superseded")
          projection.measurementsSuperseded += 1;
        if (event.type === "observation.recorded")
          projection.observationsRecorded += 1;
        if (event.type === "task.transitioned") projection.taskTransitions += 1;
        if (event.type === "override.created") {
          const id = event.payload.overrideId;
          if (typeof id === "string") projection.activeOverrides.add(id);
        }
        if (event.type === "override.reversed") {
          const id = event.payload.overrideId;
          if (typeof id === "string") projection.activeOverrides.delete(id);
        }
        return projection;
      },
      {
        status: "draft",
        measurementsRecorded: 0,
        measurementsSuperseded: 0,
        observationsRecorded: 0,
        taskTransitions: 0,
        activeOverrides: new Set<string>(),
        lastEventAt: null,
      },
    );
}

export function acknowledgeAlert(run: RunPackage, alertId: string): RunPackage {
  return touch({
    ...run,
    acknowledgedAlertIds: [...new Set([...run.acknowledgedAlertIds, alertId])],
  });
}

export function addStructuredObservation(
  run: RunPackage,
  observation: StructuredObservation,
): RunPackage {
  return touch({
    ...run,
    structuredObservations: [observation, ...run.structuredObservations],
    events: [
      {
        id: crypto.randomUUID(),
        day: dayFromTimestamp(run, observation.observedAt),
        occurredAt: observation.observedAt,
        category: "observation",
        title: `${observation.category} · ${observation.severity}`,
        detail: observation.text,
        relatedEntityId: observation.id,
      },
      ...run.events,
    ],
    auditEvents: [
      auditEvent(
        "observation-recorded",
        "observation",
        observation.id,
        observation.text,
        observation.observedAt,
      ),
      ...run.auditEvents,
    ],
    domainEvents: [
      domainEvent(
        run.id,
        "observation.recorded",
        { observationId: observation.id, category: observation.category },
        observation.observedAt,
      ),
      ...run.domainEvents,
    ],
  });
}

export function supersedeMeasurement(
  run: RunPackage,
  measurementId: string,
  correctedValue: number,
  reason: string,
  now = new Date(),
): RunPackage {
  const original = run.measurements.find((entry) => entry.id === measurementId);
  if (!original || original.supersededBy || !reason.trim()) return run;
  const occurredAt = now.toISOString();
  const correction: Measurement = {
    ...original,
    id: crypto.randomUUID(),
    measuredAt: occurredAt,
    reading: {
      ...original.reading,
      value: correctedValue,
      semantic: "measured",
      source: { kind: "manual", reference: `correction:${original.id}` },
      timestamp: occurredAt,
      quality: "verified",
    },
    correctionReason: reason.trim(),
    supersededBy: undefined,
  };
  return touch({
    ...run,
    measurements: [
      correction,
      ...run.measurements.map((entry) =>
        entry.id === original.id
          ? {
              ...entry,
              supersededBy: correction.id,
              correctionReason: reason.trim(),
            }
          : entry,
      ),
    ],
    events: [
      {
        id: crypto.randomUUID(),
        day: dayFromTimestamp(run, occurredAt),
        occurredAt,
        category: "measurement",
        title: "Messwert korrigiert",
        detail: `${original.metric}: ${String(original.reading.value)} → ${correctedValue}; ${reason.trim()}`,
        relatedEntityId: correction.id,
      },
      ...run.events,
    ],
    auditEvents: [
      auditEvent(
        "measurement-superseded",
        "measurement",
        original.id,
        `Ersetzt durch ${correction.id}: ${reason.trim()}`,
        occurredAt,
      ),
      ...run.auditEvents,
    ],
    domainEvents: [
      domainEvent(
        run.id,
        "measurement.superseded",
        { measurementId: original.id, replacementId: correction.id, reason },
        occurredAt,
      ),
      ...run.domainEvents,
    ],
  });
}

export function addRunOverride(
  run: RunPackage,
  override: RunOverride,
): RunPackage {
  if (!override.reason.trim()) return run;
  return touch({
    ...run,
    overrides: [override, ...run.overrides],
    events: [
      {
        id: crypto.randomUUID(),
        day: dayFromTimestamp(run, override.createdAt),
        occurredAt: override.createdAt,
        category: "override",
        title: `Override: ${override.field}`,
        detail: override.reason,
        relatedEntityId: override.id,
      },
      ...run.events,
    ],
    auditEvents: [
      auditEvent(
        "override-created",
        "override",
        override.id,
        `${override.field}: ${override.reason}`,
        override.createdAt,
      ),
      ...run.auditEvents,
    ],
    domainEvents: [
      domainEvent(
        run.id,
        "override.created",
        { overrideId: override.id, field: override.field },
        override.createdAt,
      ),
      ...run.domainEvents,
    ],
  });
}

export function reverseRunOverride(
  run: RunPackage,
  overrideId: string,
  now = new Date(),
): RunPackage {
  const occurredAt = now.toISOString();
  const override = run.overrides.find((entry) => entry.id === overrideId);
  if (!override || override.reversedAt) return run;
  return touch({
    ...run,
    overrides: run.overrides.map((entry) =>
      entry.id === overrideId ? { ...entry, reversedAt: occurredAt } : entry,
    ),
    auditEvents: [
      auditEvent(
        "override-reversed",
        "override",
        overrideId,
        "Override reversiert.",
        occurredAt,
      ),
      ...run.auditEvents,
    ],
    domainEvents: [
      domainEvent(run.id, "override.reversed", { overrideId }, occurredAt),
      ...run.domainEvents,
    ],
  });
}

export function deriveRunAlerts(
  run: RunPackage,
  plan: DayPlan,
  now = new Date(),
): RunAlert[] {
  const observation = latestObservation(run, plan.day);
  const alerts: RunAlert[] = [];
  if (!observation) {
    alerts.push({
      id: `day-${plan.day}-missing-measurement`,
      severity: "warning",
      title: "Aktuelle Messung fehlt",
      detail: `Für Run-Tag ${plan.day} ist kein Messdatensatz vorhanden.`,
      action: "Messwerte erfassen, bevor Planwerte verändert werden.",
      persistent: true,
    });
  } else {
    const ageHours =
      (now.getTime() - new Date(observation.recordedAt).getTime()) / 3_600_000;
    if (ageHours > 24) {
      alerts.push({
        id: `day-${plan.day}-stale-measurement`,
        severity: "warning",
        title: "Messdaten sind veraltet",
        detail: `Die letzte Messung ist ${Math.floor(ageHours)} Stunden alt.`,
        action: "Vor einer Entscheidung neu messen.",
        persistent: true,
      });
    }
    const targetPh = numberAt(plan, DAILY_COLUMNS.ph);
    const targetEc = numberAt(plan, DAILY_COLUMNS.ec);
    if (
      observation.values.phIn !== null &&
      Math.abs(observation.values.phIn - targetPh) > 0.3
    ) {
      alerts.push({
        id: `day-${plan.day}-ph-deviation`,
        severity: "critical",
        title: "pH außerhalb des Arbeitskorridors",
        detail: `Gemessen ${observation.values.phIn.toFixed(2)}, Planwert ${targetPh.toFixed(2)}.`,
        action: "Messung bestätigen und nur den finalen Endmix korrigieren.",
        persistent: true,
      });
    }
    if (
      observation.values.ecIn !== null &&
      Math.abs(observation.values.ecIn - targetEc) > 0.3
    ) {
      alerts.push({
        id: `day-${plan.day}-ec-deviation`,
        severity: "critical",
        title: "EC deutlich vom Planwert entfernt",
        detail: `Gemessen ${observation.values.ecIn.toFixed(2)}, Planwert ${targetEc.toFixed(2)} mS/cm.`,
        action:
          "Nicht blind nachdosieren; Wasser, Messgerät und Pflanzenreaktion prüfen.",
        persistent: true,
      });
    }
  }
  const config = effectiveRunConfig(run);
  if (config.water.sourcePh === null || config.water.sourceEc === null) {
    alerts.push({
      id: "water-baseline-missing",
      severity: "warning",
      title: "Wasser-Baseline unvollständig",
      detail: "Ausgangs-pH oder Ausgangs-EC fehlen in der Run-Konfiguration.",
      action:
        "Wasserprofil erfassen; keine automatische Conditioner-Dosis ableiten.",
      persistent: true,
    });
  }
  return alerts;
}

export function validateRunPackage(
  value: unknown,
): { ok: true; value: RunPackage } | { ok: false; errors: string[] } {
  const errors: string[] = [];
  if (!value || typeof value !== "object")
    return { ok: false, errors: ["Datei enthält kein Objekt."] };
  const migrated = migrateRunPackage(value);
  if (!migrated)
    return {
      ok: false,
      errors: [
        `Schema ${String((value as { schemaVersion?: unknown }).schemaVersion)} wird nicht unterstützt.`,
      ],
    };
  const candidate: Partial<RunPackage> = migrated;
  if (!candidate.id || typeof candidate.id !== "string")
    errors.push("Run-ID fehlt.");
  if (!candidate.config || typeof candidate.config !== "object")
    errors.push("Run-Konfiguration fehlt.");
  if (!Array.isArray(candidate.observations))
    errors.push("Messwertliste fehlt.");
  if (!Array.isArray(candidate.events)) errors.push("Ereignisliste fehlt.");
  if (!candidate.completedTasks || typeof candidate.completedTasks !== "object")
    errors.push("Checklistenstatus fehlt.");
  if (!Array.isArray(candidate.acknowledgedAlertIds))
    errors.push("Alertstatus fehlt.");
  if (!Array.isArray(candidate.inventory)) errors.push("Bestandslog fehlt.");
  if (!candidate.configurationSnapshot)
    errors.push("Konfigurations-Snapshot fehlt.");
  if (!Array.isArray(candidate.zones)) errors.push("Zonenliste fehlt.");
  if (!Array.isArray(candidate.plants)) errors.push("Pflanzenliste fehlt.");
  if (!Array.isArray(candidate.measurements))
    errors.push("Typisierte Messwertliste fehlt.");
  if (!Array.isArray(candidate.structuredObservations))
    errors.push("Beobachtungsliste fehlt.");
  if (!Array.isArray(candidate.tasks)) errors.push("Aufgabenliste fehlt.");
  if (!Array.isArray(candidate.overrides)) errors.push("Override-Liste fehlt.");
  if (!Array.isArray(candidate.auditEvents)) errors.push("Audit-Log fehlt.");
  if (!Array.isArray(candidate.domainEvents))
    errors.push("Domain-Event-Log fehlt.");
  if (!Array.isArray(candidate.devices)) errors.push("Geräteliste fehlt.");
  if (!Array.isArray(candidate.calibrations))
    errors.push("Kalibrierungsliste fehlt.");
  if (!Array.isArray(candidate.growthEvents))
    errors.push("Growth-Events fehlen.");
  if (!Array.isArray(candidate.irrigationEvents))
    errors.push("Bewässerungs-Events fehlen.");
  if (!Array.isArray(candidate.mixBatches)) errors.push("Mix-Batches fehlen.");
  if (!Array.isArray(candidate.reservoirs))
    errors.push("Reservoirliste fehlt.");
  if (!Array.isArray(candidate.equipment)) errors.push("Equipmentliste fehlt.");
  if (!Array.isArray(candidate.maintenanceEvents))
    errors.push("Wartungsevents fehlen.");
  if (!Array.isArray(candidate.ipmInspections))
    errors.push("IPM-Inspektionen fehlen.");
  if (!Array.isArray(candidate.incidents)) errors.push("Incidents fehlen.");
  if (!Array.isArray(candidate.postHarvest))
    errors.push("Post-Harvest-Daten fehlen.");
  if (!Array.isArray(candidate.energyReadings))
    errors.push("Energiedaten fehlen.");
  if (!Array.isArray(candidate.cultivarProfiles))
    errors.push("Cultivar-Profile fehlen.");
  if (!Array.isArray(candidate.productInventory))
    errors.push("Produktinventar fehlt.");
  if (!Array.isArray(candidate.nutrientSystems))
    errors.push("Nährstoffsysteme fehlen.");
  if (!Array.isArray(candidate.mixApplications))
    errors.push("Mix-Applikationen fehlen.");
  if (!Array.isArray(candidate.mediaAssets))
    errors.push("Medienmetadaten fehlen.");
  if (!candidate.storageState || typeof candidate.storageState !== "object")
    errors.push("Storage-Zustand fehlt.");
  if (
    candidate.executionMode !== "simulation" &&
    candidate.executionMode !== "live"
  )
    errors.push("Ausführungsmodus fehlt oder ist ungültig.");
  if (!Array.isArray(candidate.anchorRevisions))
    errors.push("Ankerkorrekturen fehlen.");
  if (!candidate.clockHealth || typeof candidate.clockHealth !== "object")
    errors.push("Clock-Health fehlt.");
  if (!candidate.backupState || typeof candidate.backupState !== "object")
    errors.push("Backupstatus fehlt.");
  if (!Array.isArray(candidate.aiProposalReviews))
    errors.push("AI-Prüfprotokoll fehlt.");
  if (!Array.isArray(candidate.backupCheckpoints))
    errors.push("Backup-Checkpoints fehlen.");
  return errors.length > 0
    ? { ok: false, errors }
    : { ok: true, value: migrated };
}

export function migrateRunPackage(value: unknown): RunPackage | null {
  if (!value || typeof value !== "object") return null;
  let candidate = value as Record<string, unknown>;

  if (candidate.schemaVersion === RUN_SCHEMA_VERSION) {
    if (candidate.config && (candidate.config as any).genetics === "Double Grape Auto") {
      (candidate.config as any).genetics = "Double Grape";
    }
    return candidate as unknown as RunPackage;
  }

  const importedAt = new Date().toISOString();

  if (candidate.schemaVersion === "1.0.0") {
    if (!candidate.config || typeof candidate.config !== "object") return null;
    const createdAt =
      typeof candidate.createdAt === "string"
        ? candidate.createdAt
        : importedAt;
    const base = createDefaultRunPackage(new Date(createdAt));
    const oldConfig = candidate.config as any;

    const config: RunConfig = {
      ...base.config,
      ...oldConfig,
      water: {
        ...base.config.water,
        ...(oldConfig.water || {}),
      },
    };

    const observations = Array.isArray(candidate.observations)
      ? (candidate.observations as DailyObservation[])
      : [];
    const v2Id = typeof candidate.id === "string" ? candidate.id : base.id;

    candidate = {
      ...base,
      id: v2Id,
      schemaVersion: "2.0.0",
      createdAt,
      updatedAt:
        typeof candidate.updatedAt === "string"
          ? candidate.updatedAt
          : createdAt,
      config,
      ...synchronizedAssets(base, config),
      configurationSnapshot: {
        ...base.configurationSnapshot,
        capturedAt: createdAt,
        config: copyConfig(config),
      },
      observations,
      events: Array.isArray(candidate.events) ? candidate.events : [],
      completedTasks:
        candidate.completedTasks && typeof candidate.completedTasks === "object"
          ? candidate.completedTasks
          : {},
      acknowledgedAlertIds: Array.isArray(candidate.acknowledgedAlertIds)
        ? candidate.acknowledgedAlertIds
        : [],
      inventory: Array.isArray(candidate.inventory) ? candidate.inventory : [],
      auditEvents: [
        auditEvent(
          "imported",
          "run",
          v2Id,
          "RunPackage v1 verlustfrei auf v2 migriert.",
        ),
        ...base.auditEvents,
      ],
      domainEvents: [
        domainEvent(
          v2Id,
          "run.imported",
          { fromSchema: "1.0.0", toSchema: "2.0.0" },
          createdAt,
          "migration",
        ),
        ...base.domainEvents,
      ],
    } as any;

    (candidate as any).measurements = observations.flatMap((obs) =>
      measurementsFromObservation(candidate as unknown as RunPackage, obs),
    );
  }

  if (candidate.schemaVersion === "2.0.0") {
    candidate = {
      ...candidate,
      schemaVersion: "3.0.0",
      devices: [],
      calibrations: [],
      measurements: ((candidate.measurements as Measurement[]) ?? []).map(
        (measurement) => ({
          ...measurement,
          trustStatus:
            measurement.reading.quality === "verified" ? "valid" : "unverified",
        }),
      ),
      domainEvents: [
        domainEvent(
          candidate.id as string,
          "run.imported",
          { fromSchema: "2.0.0", toSchema: "3.0.0" },
          importedAt,
          "migration",
        ),
        ...((candidate.domainEvents as DomainEvent[]) || []),
      ],
      privacyClassification: "project",
    };
  }

  if (candidate.schemaVersion === "3.0.0") {
    const oldConfig = candidate.config as any;
    const config: RunConfig = {
      ...oldConfig,
      dayZeroAnchor: "run-operational-start",
      mediumProduct: "",
      water: {
        ...defaultWaterProfile(),
        sourcePh: oldConfig.water?.sourcePh ?? null,
        sourceEc: oldConfig.water?.sourceEc ?? null,
        calciumMgL: oldConfig.water?.calciumMgL ?? null,
        magnesiumMgL: oldConfig.water?.magnesiumMgL ?? null,
        alkalinityMgL: oldConfig.water?.alkalinityMgL ?? null,
      },
      pot: {
        type: "other",
        nominalVolumeLiters: 9,
        actualFillLiters: null,
        diameterCm: null,
        heightCm: null,
        emptyMassGrams: null,
        saturatedMassGrams: null,
      },
      light: null,
    };

    const plants = ((candidate.plants as any[]) || []).map((p) => ({
      ...p,
      identity: defaultPlantIdentity(),
    }));

    candidate = {
      ...candidate,
      schemaVersion: "4.0.0",
      config,
      configurationSnapshot: {
        ...(candidate.configurationSnapshot as any),
        config: copyConfig(config),
      },
      plants,
      growthEvents: [],
      irrigationEvents: [],
      mixBatches: [],
      reservoirs: [],
      equipment: [],
      maintenanceEvents: [],
      lungRoom: null,
      ipmInspections: [],
      incidents: [],
      postHarvest: [],
      energyReadings: [],
      cultivarProfiles: [],
      productInventory: [],
      nutrientSystems: [],
      domainEvents: [
        domainEvent(
          candidate.id as string,
          "run.imported",
          { fromSchema: "3.0.0", toSchema: "4.0.0" },
          importedAt,
          "migration",
        ),
        ...((candidate.domainEvents as DomainEvent[]) || []),
      ],
    };
  }

  if (candidate.schemaVersion === "4.0.0") {
    const occurredAt = importedAt;
    candidate = {
      ...candidate,
      schemaVersion: "5.0.0",
      calibrations: ((candidate.calibrations as any[]) || []).map(
        (calibration) => ({
          ...calibration,
          points: calibration.points ?? [],
          referenceLot: calibration.referenceLot ?? null,
          temperatureC: calibration.temperatureC ?? null,
          electrodeCondition: calibration.electrodeCondition ?? "unknown",
          revision: calibration.revision ?? 1,
        }),
      ),
      mixBatches: ((candidate.mixBatches as any[]) || []).map((batch) => ({
        ...batch,
        revision: batch.revision ?? 1,
      })),
      reservoirs: ((candidate.reservoirs as any[]) || []).map((reservoir) => ({
        ...reservoir,
        recordedAt: reservoir.recordedAt ?? occurredAt,
        revision: reservoir.revision ?? 1,
      })),
      equipment: ((candidate.equipment as any[]) || []).map((equipment) => ({
        ...equipment,
        status: equipment.status ?? "active",
        version: equipment.version ?? 1,
        revision: equipment.revision ?? 1,
      })),
      ipmInspections: ((candidate.ipmInspections as any[]) || []).map(
        (inspection) => ({
          ...inspection,
          status:
            inspection.status ?? (inspection.closedAt ? "closed" : "open"),
          followUpCompleted:
            inspection.followUpCompleted ?? Boolean(inspection.closedAt),
          updatedAt: inspection.updatedAt ?? inspection.inspectedAt,
          revision: inspection.revision ?? 1,
        }),
      ),
      incidents: ((candidate.incidents as any[]) || []).map((incident) => ({
        ...incident,
        updatedAt: incident.updatedAt ?? incident.detectedAt,
        revision: incident.revision ?? 1,
      })),
      postHarvest: ((candidate.postHarvest as any[]) || []).map((record) => ({
        ...record,
        status:
          record.status ??
          (record.storageStartedAt
            ? "stored"
            : record.cureStartedAt
              ? "curing"
              : record.dryingStartedAt
                ? "drying"
                : "planned"),
        revision: record.revision ?? 1,
      })),
      productInventory: ((candidate.productInventory as any[]) || []).map(
        (item) => ({ ...item, revision: item.revision ?? 1 }),
      ),
      nutrientSystems: ((candidate.nutrientSystems as any[]) || []).map(
        (system) => ({
          ...system,
          status: system.status ?? "reference",
          ruleBundleVersion: system.ruleBundleVersion ?? null,
          revision: system.revision ?? 1,
        }),
      ),
      mixApplications: [],
      mediaAssets: [],
      storageState: {
        state: "healthy",
        lastSuccessfulWriteAt:
          typeof candidate.updatedAt === "string"
            ? candidate.updatedAt
            : occurredAt,
        lastFailureAt: null,
        failureKind: null,
        detail: null,
        unsavedChanges: false,
      },
      domainEvents: [
        domainEvent(
          candidate.id as string,
          "run.imported",
          { fromSchema: "4.0.0", toSchema: "5.0.0" },
          occurredAt,
          "migration",
        ),
        ...((candidate.domainEvents as DomainEvent[]) || []),
      ],
    };
  }

  if (candidate.schemaVersion === "5.0.0") {
    const occurredAt = importedAt;
    candidate = {
      ...candidate,
      schemaVersion: "6.0.0",
      executionMode: "simulation",
      sourceSimulationRunId: null,
      liveAnchor: null,
      anchorRevisions: [],
      clockHealth: {
        status: "healthy",
        lastObservedAtUtc:
          typeof candidate.updatedAt === "string"
            ? candidate.updatedAt
            : occurredAt,
        detail: null,
      },
      backupState: {
        pending: false,
        lastCheckpointAtUtc: null,
        lastCheckpointSha256: null,
        lastVerifiedAtUtc: null,
        lastExternalBackupAtUtc: null,
        externalMode: "none",
        persistentStorage: "unknown",
        usageBytes: null,
        quotaBytes: null,
      },
      aiProposalReviews: [],
      backupCheckpoints: [],
      domainEvents: [
        domainEvent(
          candidate.id as string,
          "run.imported",
          { fromSchema: "5.0.0", toSchema: "6.0.0" },
          occurredAt,
          "migration",
        ),
        ...((candidate.domainEvents as DomainEvent[]) || []),
      ],
    };
  }

  if (candidate.schemaVersion === RUN_SCHEMA_VERSION) {
    return candidate as unknown as RunPackage;
  }

  return null;
}

export function inventoryBalance(run: RunPackage): number {
  return run.inventory.reduce((sum, event) => {
    const outgoing =
      event.type === "consumption" || event.type === "destruction";
    return sum + (outgoing ? -Math.abs(event.grams) : event.grams);
  }, 0);
}

export function runToCsv(run: RunPackage): string {
  const header = [
    "day",
    "recordedAt",
    "source",
    ...Object.keys(emptyValues()),
    "notes",
  ];
  const csvEscape = (value: unknown) =>
    `"${String(value ?? "").replaceAll('"', '""')}"`;
  const rows = run.observations.map((entry) => [
    entry.day,
    entry.recordedAt,
    entry.source,
    ...Object.values(entry.values),
    entry.notes,
  ]);
  return [header, ...rows]
    .map((row) => row.map(csvEscape).join(","))
    .join("\r\n");
}

function touch(run: RunPackage): RunPackage {
  return { ...run, updatedAt: new Date().toISOString() };
}

function copyConfig(config: RunConfig): RunConfig {
  return JSON.parse(JSON.stringify(config));
}

function defaultWaterProfile(): WaterProfile {
  return {
    sourceType: "unknown",
    sourceDescription: "",
    sourcePh: null,
    sourceEc: null,
    calciumMgL: null,
    magnesiumMgL: null,
    alkalinityMgL: null,
    sodiumMgL: null,
    chlorideMgL: null,
    sulfateMgL: null,
    analysisDate: null,
    analysisSource: null,
    analysisMethod: "unknown",
    verified: false,
  };
}

function defaultPlantIdentity(): PlantIdentity {
  return {
    breeder: null,
    seedType: "unknown",
    seedLot: null,
    packBatch: null,
    sourceDate: null,
    phenotypeNotes: "",
  };
}

function synchronizedAssets(
  run: RunPackage,
  config: RunConfig,
): Pick<RunPackage, "zones" | "plants"> {
  const primaryZone = run.zones[0] ?? {
    id: crypto.randomUUID(),
    name: "Hauptzone",
    kind: "tent" as const,
    widthCm: null,
    depthCm: null,
    heightCm: null,
  };
  const zones = [
    {
      ...primaryZone,
      widthCm: config.tentWidthCm,
      depthCm: config.tentDepthCm,
      heightCm: config.tentHeightCm,
    },
    ...run.zones.slice(1),
  ];
  const count = Math.max(0, Math.round(config.plantCount));
  const plants = Array.from({ length: count }, (_, index) => {
    const existing = run.plants[index];
    return existing
      ? {
          ...existing,
          zoneId: primaryZone.id,
          genetics: config.genetics,
        }
      : {
          id: crypto.randomUUID(),
          zoneId: primaryZone.id,
          label: `Pflanze ${String.fromCharCode(65 + (index % 26))}${Math.floor(index / 26) + 1}`,
          genetics: config.genetics,
          status: "planned" as const,
          identity: defaultPlantIdentity(),
        };
  });
  return { zones, plants };
}

function auditEvent(
  action: AuditEvent["action"],
  entityType: string,
  entityId: string,
  detail: string,
  occurredAt = new Date().toISOString(),
): AuditEvent {
  return {
    id: crypto.randomUUID(),
    occurredAt,
    action,
    entityType,
    entityId,
    detail,
  };
}

function domainEvent(
  aggregateId: string,
  type: DomainEventType,
  payload: Record<string, unknown>,
  occurredAt = new Date().toISOString(),
  actor: DomainEvent["actor"] = "system",
): DomainEvent {
  return {
    id: crypto.randomUUID(),
    schemaVersion: "1.0.0",
    aggregateType: "run",
    aggregateId,
    type,
    occurredAt,
    recordedAt: new Date().toISOString(),
    actor,
    source: "ukd-domain",
    payload,
  };
}

function measurementsFromObservation(
  run: RunPackage,
  observation: DailyObservation,
): Measurement[] {
  const zoneId = run.zones[0]?.id ?? "unassigned-zone";
  const plantId = run.plants.length === 1 ? run.plants[0]?.id : undefined;
  return measurementMap.flatMap(({ key, metric, unit }) => {
    const value = observation.values[key];
    if (value === null) return [];
    return [
      {
        id: crypto.randomUUID(),
        runId: run.id,
        zoneId,
        ...(plantId ? { plantId } : {}),
        metric,
        reading: {
          value,
          ...(unit ? { unit } : {}),
          semantic: "measured" as const,
          source: {
            kind: observation.source,
            reference: `daily-observation:${observation.id}`,
          },
          timestamp: observation.recordedAt,
          quality: "unverified" as const,
          stale: observation.status === "stale",
        },
        measuredAt: observation.recordedAt,
        trustStatus: observation.source === "sensor" ? "unverified" : "valid",
        calibrationState:
          observation.source === "sensor" ? "unknown" : undefined,
      },
    ];
  });
}

function dayFromTimestamp(run: RunPackage, timestamp: string): number {
  const start = new Date(`${effectiveRunConfig(run).startDate}T00:00:00Z`);
  const value = new Date(timestamp);
  if (Number.isNaN(start.getTime()) || Number.isNaN(value.getTime())) return 0;
  return Math.max(
    0,
    Math.round((value.getTime() - start.getTime()) / 86_400_000),
  );
}

function taskKey(day: number, title: string): string {
  const slug = title
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-|-$/g, "");
  return `day-${day}:${slug || "task"}`;
}
