    ...
    <div className="page-stack">
      <section className="workspace-banner">
        <div>
          ...
          <p>
            Befund, Schweregrad, Maßnahmen, Follow-up und Ergebnis bleiben als
            Zustandsverlauf erhalten.
          </p>
        </div>
      </section>
      <form className="panel form-panel" onSubmit={submit}>
        <header>
          <div>
            <small>NEUER BEFUND</small>
            <h2>Inspektion erfassen</h2>
          </div>
        </header>
        <div className="form-grid">
          ...
          <label>
            ...
            <select
              value={severity}
              onChange={(event) =>
                setSeverity(event.target.value as IpmSeverity)
              }
            >
              {["none", "trace", "low", "moderate", "severe", "critical"].map(
                (value) => (
      ...
      <section className="panel">
        <header>
          <div>
            ...
            <h2>Offene und abgeschlossene Befunde</h2>
          </div>
        </header>
        <div className="record-list">
          {run.ipmInspections.length === 0 ? (
            <p>Noch keine IPM-Inspektion.</p>
          ) : (
            run.ipmInspections.map((entry) => (
              <article className="record-card" key={entry.id}>
                <div>
                  <small>
                    TAG {entry.day} · {(entry.status ?? "open").toUpperCase()}
                  </small>
                  <h3>{entry.finding}</h3>
                  <p>
                    {entry.location} · {entry.severity} · Revision{" "}
                    {entry.revision ?? 1}
                  </p>
                </div>
                {entry.status !== "closed" && (
                  <button
                    type="button"
                    onClick={() => advance(entry.id, entry.status ?? "open")}
                  >
                    Nächsten Status bestätigen
                  </button>
                )}
              </article>
            ))
...
export function IncidentWorkspace({ run, lens, onChange }: OperationsProps) {
    ...
    if (
      dispatch({
        ...
        incident: {
          ...
          affectedPlantIds: [],
          planSuperseded: severity === "high" || severity === "critical",
        },
      })
    )
      setDescription("");
  ...
  const advance = (id: string, status: IncidentStatus) => {
    ...
    dispatch({
      ...
      incidentId: id,
      status: next,
      reason: "Operativer Recovery-Checkpoint bestätigt",
      action:
        next === "mitigating"
          ? { action: "Sofortmaßnahme eingeleitet", result: "Recovery läuft" }
          : undefined,
      rootCause:
        next === "closed"
          ? "Root Cause im Recovery-Review bestätigt"
          : undefined,
      lessonsLearned:
        next === "closed"
          ? "Präventionsmaßnahme in Wartung und Run-Review übernommen"
          : undefined,
    });
  };
  return (
    <div className="page-stack">
      <section className="workspace-banner">
        <div>
          <small>FAILURE UX · {lens.toUpperCase()}</small>
          <h2>Incidents & Recovery</h2>
          <p>
            Kritische Incidents superseden den Tagesplan, bis der
            Recovery-Workflow abgeschlossen ist.
          </p>
        </div>
      </section>
      <form className="panel form-panel" onSubmit={submit}>
        <header>
          <div>
            <small>NEUER INCIDENT</small>
            <h2>Störfall eröffnen</h2>
          </div>
        </header>
        <div className="form-grid">
          <label>
      ...
      <section className="panel">
        <header>
          <div>
            <small>RECOVERY STATE MACHINE</small>
            <h2>Incident-Verlauf</h2>
          </div>
        </header>
        <div className="record-list">
          {run.incidents.length === 0 ? (
            ...
            run.incidents.map((entry) => (
              <article className="record-card" key={entry.id}>
                <div>
                  <small>
                    {entry.category} · {entry.status.toUpperCase()}
                  </small>
                  <h3>{entry.description}</h3>
                  <p>
                    Severity {entry.severity} · Plan superseded:{" "}
                    {entry.planSuperseded ? "JA" : "NEIN"}
                  </p>
                </div>
                {entry.status !== "closed" && (
                  <button
...
export function EquipmentOperationsWorkspace({
  run,
  lens,
  onChange,
}: OperationsProps) {
  ...
  const maintain = () =>
    ...
    dispatch({
      ...
      maintenance: {
        ...
        type: "inspection" as MaintenanceEventType,
        performedAt: new Date().toISOString(),
        performedBy: "Local operator",
        nextDueAt: null,
        result: "passed",
        notes: "Operative Sichtprüfung",
        cost: null,
        currency: "EUR",
      },
    });
  return (
    <section className="panel form-panel">
      <header>
        <div>
          <small>ASSET HISTORY · {lens.toUpperCase()}</small>
          <h2>Equipment- und Wartungshistorie</h2>
        </div>
      </header>
      <form onSubmit={add}>
        <div className="form-grid">
      ...
      {run.equipment.length > 0 && (
        <div className="form-grid">
          <label>
            <span>Asset für Sichtprüfung</span>
            <select
              value={maintenanceEquipmentId}
              onChange={(event) =>
                setMaintenanceEquipmentId(event.target.value)
              }
            >
              <option value="">Auswählen</option>
              {run.equipment.map((entry) => (
      ...
      <div className="record-list">
        {run.equipment.map((entry) => (
          <article className="record-card" key={entry.id}>
            <div>
              <small>
                {entry.category} · {(entry.status ?? "active").toUpperCase()}
              </small>
              <h3>
                {entry.manufacturer} {entry.model}
              </h3>
              <p>
                Revision {entry.revision ?? 1} · Wartungen{" "}
                {
                  run.maintenanceEvents.filter(
                    (event) => event.equipmentId === entry.id,
                  ).length
                }
              </p>
            </div>
...
export function ProductInventoryWorkspace({
  ...
  return (
    <div className="page-stack">
      <section className="workspace-banner">
        <div>
          <small>MY INVENTORY · {lens.toUpperCase()}</small>
          <h2>Produktinventar</h2>
          <p>
            Persönlicher Bestand bleibt vom kanonischen Knowledge Catalog
            getrennt.
          </p>
        </div>
      </section>
      ...
      <section className="panel">
        <div className="record-list">
          {run.productInventory.length === 0 ? (
            <p>Noch kein persönlicher Produktbestand.</p>
          ) : (
            run.productInventory.map((entry) => (
              <article className="record-card" key={entry.id}>
                <div>
                  <small>
                    {entry.owned ? "VORHANDEN" : "NICHT VORHANDEN"} · REV{" "}
                    {entry.revision ?? 1}
                  </small>
                  <h3>{entry.productName}</h3>
                  <p>
                    {entry.containerSize} · Rest{" "}
                    {entry.remainingEstimate ?? "nicht erfasst"}
                  </p>
                </div>
              </article>
            ))
