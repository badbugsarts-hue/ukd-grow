...
export function validateRunConfig(config: RunConfig): ValidationIssue[] {
	...
	const activeSubstrate = productDatabase.substrates.find((s) =>
	...
	if (activeNutrient && activeSubstrate) {
		if (activeNutrient.compatibility.notRecommendedSubstrates.includes(activeSubstrate.category)) {
			issues.push({
				level: "danger",
	...
	if (activeNutrient && activeIrrigation) {
		if (activeNutrient.compatibility.irrigationWarning === "drip" && activeIrrigation.type === "drip") {
			issues.push({
				level: "warning",
