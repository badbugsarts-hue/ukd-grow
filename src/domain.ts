...
export const DAILY_COLUMNS = {
  ...
  plantDayP3: 62,
  phReferenceMin: 63,
  phWorkingSetpoint: 64,
  phReferenceMax: 65,
  phStateSource: 66,
  athenaDecision: 67,
  clockPhPolicyNote: 68,
} as const;
...
export function calculateMix(plan: DayPlan, batchLiters: number): MixItem[] {
  ...
  const items: Array<[string, number, string, string?]> = [
    ...
    ["Tarantula", DAILY_COLUMNS.tarantula, "Frisch/manuell"],
    [
      "pH Down",
...
export function calculateBiologicalPlantAge(
  ...
  if (seedEvent && emergenceEv) {
    ...
    if (!Number.isNaN(sTime) && !Number.isNaN(eTime)) {
      germinationDays = Math.max(
        0,
        Math.floor((eTime - sTime) / (1000 * 60 * 60 * 24)),
      );
    }
  }
...
export interface PlantBatchStatus {
  plantId: string;
  label: string;
  genetics: string;
  biologicalAgeDays: number;
  stage:
    "pre-emergence" | "seedling" | "veg" | "preflower" | "bloom" | "harvest";
  assignedBatch: "blocked" | "common" | "bloom_split" | "water_only";
  rootStage: "unknown" | "plug" | "block" | "final-coco";
  workingPh: 5.8 | 6.0;
  feedEligible: boolean;
  stageSource: "confirmed-event" | "missing-event";
  blockers: string[];
}

export function resolvePlantBatches(
  plants: Array<{ id: string; label: string; genetics: string | null }>,
  growthEvents: GrowthEvent[],
  dayZeroAnchor: DayZeroAnchor,
  now = new Date(),
): PlantBatchStatus[] {
  return plants.map((plant) => {
    const plantEvents = growthEvents.filter((e) => e.plantId === plant.id);
    ...
    const age = calculateBiologicalPlantAge(dayZeroAnchor, plantEvents, now);
    const blockers: string[] = [];
    let stage: PlantBatchStatus["stage"] = "pre-emergence";
    if (confirmed.has("emergence")) stage = "seedling";
    if (
      confirmed.has("nutrient-uptake-ready") ||
      confirmed.has("first-true-leaves")
    )
      stage = "veg";
    if (confirmed.has("preflower-observed")) stage = "preflower";
    if (confirmed.has("flower-onset") || confirmed.has("late-flower"))
      stage = "bloom";
    if (confirmed.has("harvest-ready") || confirmed.has("harvested"))
      stage = "harvest";

    let rootStage: PlantBatchStatus["rootStage"] = "unknown";
    if (confirmed.has("root-zone-plug")) rootStage = "plug";
    if (confirmed.has("root-zone-block")) rootStage = "block";
    if (confirmed.has("root-zone-final-coco")) rootStage = "final-coco";
    if (!confirmed.has("emergence"))
      blockers.push("Durchbruch noch nicht bestätigt");
    if (
      !confirmed.has("nutrient-uptake-ready") &&
      !confirmed.has("first-true-leaves")
    )
      blockers.push("Nährstoffaufnahme noch nicht bestätigt");
    if (rootStage === "unknown") blockers.push("Wurzelzonen-Stufe fehlt");
    if (confirmed.has("harvested")) blockers.push("Pflanze bereits geerntet");
    const feedEligible = blockers.length === 0;
    let assignedBatch: PlantBatchStatus["assignedBatch"] = "blocked";
    if (feedEligible)
      assignedBatch =
        stage === "bloom" || stage === "harvest" ? "bloom_split" : "common";
    if (confirmed.has("harvested")) assignedBatch = "water_only";

    return {
      plantId: plant.id,
      label: plant.label,
      genetics: plant.genetics || "Unbekannt",
      biologicalAgeDays: age.biologicalAgeDays,
      stage,
      assignedBatch,
      rootStage,
      workingPh: rootStage === "final-coco" ? 6.0 : 5.8,
      feedEligible,
      stageSource: confirmed.size > 0 ? "confirmed-event" : "missing-event",
      blockers,
    };
  });
}
