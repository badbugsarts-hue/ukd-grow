...
describe("Milestone 2 — Autoflower Cockpit Browser & Selector Suite", () => {
	describe("1. Dataset & Provenance Integrity", () => {
		...
		it("validates that every strain has complete 44-attribute profile", () => {
			...
			for (const strain of cockpitData) {
				for (const field of requiredFields) {
					expect(strain[field], `Strain ${strain.name} missing ${String(field)}`).toBeDefined();
				}
				...
				expect(["Autoflower", "Photoperiodisch", "Fast Version"]).toContain(strain.typ);
			}
		});
		...
		it("accurately classifies original breeder lines, white label lots, and unklar listings", () => {
			const originalBreederStrains = cockpitData.filter(
				(s) => (s.breeder === "Sensi Seeds" || s.breeder === "Fast Buds" || s.breeder === "Mephisto Genetics") &&
					s.name !== "Super Boof XL Auto" && !s.id.includes("super-boof"),
			);
	...
	describe("3. Filtering Logic Verification", () => {
		it("filters correctly by kind (Jungpflanze vs. Saatgut)", () => {
			const jungpflanzenOnly = cockpitData.filter((s) => s.kind === "jungpflanze");
		...
		it("filters correctly by multi-attribute fulltext search", () => {
			...
			expect(matches.some((s) => s.name.includes("Double Grape") || s.name.includes("Sour Stomper"))).toBe(true);
		});

		it("filters correctly by breeder seedbank", () => {
			const sensiStrains = cockpitData.filter((s) => s.breeder === "Sensi Seeds");
			expect(sensiStrains.length).toBeGreaterThanOrEqual(7);

			const fastBudsStrains = cockpitData.filter((s) => s.breeder === "Fast Buds");
		...
		it("filters correctly by maximum canopy height", () => {
			const maxHeight = 80;
			const compactStrains = cockpitData.filter((s) => s.hmax !== null && s.hmax <= maxHeight);
	...
	describe("4. Sorting Logic Verification", () => {
		it("sorts correctly by Masterclass score descending", () => {
			...
			for (let i = 0; i < sortedByScore.length - 1; i++) {
				expect(sortedByScore[i].score).toBeGreaterThanOrEqual(sortedByScore[i + 1].score);
			}
		});
		...
		it("sorts correctly by dry yield potential descending", () => {
			...
			for (let i = 0; i < sortedByYield.length - 1; i++) {
				const avgCurrent = (sortedByYield[i].ertrag_lo + sortedByYield[i].ertrag_hi) / 2;
				const avgNext = (sortedByYield[i + 1].ertrag_lo + sortedByYield[i + 1].ertrag_hi) / 2;
				expect(avgCurrent).toBeGreaterThanOrEqual(avgNext);
			}
		});

		it("sorts correctly by alphabetical cultivar name", () => {
			const sortedByName = [...cockpitData].sort((a, b) => a.name.localeCompare(b.name, "de"));
			for (let i = 0; i < sortedByName.length - 1; i++) {
				expect(sortedByName[i].name.localeCompare(sortedByName[i + 1].name, "de")).toBeLessThanOrEqual(0);
			}
		});
	...
	describe("5. Yield Uncertainty & Photobiology Modeling", () => {
		it("verifies the 140 W / 0.36 m² tent yield scaling model across all strains", () => {
			for (const strain of cockpitData) {
				...
				expect(strain.ertrag_lo).toBeGreaterThanOrEqual(Math.floor(expectedMinBasis) - 10);
				expect(strain.ertrag_hi).toBeLessThanOrEqual(Math.ceil(expectedMaxBasis) + 10);
			}
		});
	});
