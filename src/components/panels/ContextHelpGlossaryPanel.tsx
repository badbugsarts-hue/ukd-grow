		...
		<div className="page-stack">
			<section className="workspace-banner">
				<div>
					...
					<p>
						Alle Definitionen stammen aus der versionierten Knowledge Base. Diese
						Ansicht enthält keine eigenen Zielwerte oder zweite fachliche Wahrheit.
					</p>
				</div>
				<LensBadge lens={lens} />
			</section>
					...
					{terms.map((term) => (
						<article className="record-card" key={term.key}>
							<div>
								...
								<h3>{term.acronym} · {term.germanName}</h3>
								<p>
									{lens === "expert"
										? term.expert
										: lens === "advanced"
								...
								{lens === "expert" && (
									<small>Quellen: {term.sourceIds.join(", ") || "kanonische Definition"}</small>
								)}
							</div>
						</article>
					))}
