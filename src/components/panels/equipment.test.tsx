...
describe("Milestone 2 - Equipment Manager & Calibration Component Suite", () => {
	...
	it("3. verifies sensor calibration status calculations and history recording", () => {
		...
		const initialPhStatus = getSensorCalibrationStatus("ph-sensor-main", "water.ph", run.calibrations);
		...
		const newPhStatus = getSensorCalibrationStatus("ph-sensor-main", "water.ph", updatedRun.calibrations);
		expect(newPhStatus).toBe("VALID");

		// Test expired calibration (performed 40 days ago without validUntil, default window 30 days)
		const pastDate = new Date(now.getTime() - 40 * 24 * 60 * 60 * 1000).toISOString();
		...
		const expiredStatus = getSensorCalibrationStatus("ph-sensor-main", "water.ph", expiredRun.calibrations);
	...
	it("4. verifies EC sensor calibration status windows (60 days)", () => {
		...
		const recentPast = new Date(now.getTime() - 40 * 24 * 60 * 60 * 1000).toISOString();
		...
		const ecStatus = getSensorCalibrationStatus("ec-sensor-main", "water.ec", ecRun.calibrations, now);
	...
	it("integrates EC sensor calibration limits", () => {
		...
		const ecStatus = getSensorCalibrationStatus("ec-sensor-main", "water.ec", ecRun.calibrations);
		expect(ecStatus).toBe("VALID");
	});
});
