...
export const EquipmentManagerPanel: React.FC<EquipmentManagerPanelProps> = ({
  ...
  const phDeviceId =
    run.calibrations.find((record) => record.metric === "water.ph")?.deviceId ??
    "";
  const ecDeviceId =
    run.calibrations.find((record) => record.metric === "water.ec")?.deviceId ??
    "";
  const phStatus = phDeviceId
    ? getSensorCalibrationStatus(phDeviceId, "water.ph", run.calibrations)
    : "UNKNOWN";
  const ecStatus = ecDeviceId
    ? getSensorCalibrationStatus(ecDeviceId, "water.ec", run.calibrations)
    : "UNKNOWN";
      ...
      >
        ...
        >
          <div>
            ...
            >
              Prüfe das erfasste Lichtprofil, Kalibrierungen deiner Messgeräte
              für <TermTooltip term="pH" lens={lens} /> &{" "}
              <TermTooltip term="EC" lens={lens} /> und führe präzise 9-Punkt-{" "}
              <TermTooltip term="PPFD" lens={lens} /> Rastermessungen durch. Ein
              vollständiges Hardware-Inventar ist noch nicht implementiert.
            </p>
          </div>
          <LensBadge lens={lens} />
        </div>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              Software-Fallback: 30 Tage. Hersteller-, Labor- und dokumentierte
              Gerätevorgaben haben Vorrang.
            </p>
            <button
              type="button"
              onClick={() => {
          ...
          >
            ...
            >
              Software-Fallback: 60 Tage. Hersteller-, Labor- und dokumentierte
              Gerätevorgaben haben Vorrang.
            </p>
            <button
              type="button"
