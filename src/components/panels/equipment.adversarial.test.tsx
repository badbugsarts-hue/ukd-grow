...
describe("Empirical Adversarial Challenge Suite - Equipment & Calibration M2 Components", () => {
	...
	describe("2. Sensor Calibration Temporal & Boundary Oracles", () => {
		it("returns 'uncalibrated' if calibrations array has no matching device", () => {
			...
			expect(getSensorCalibrationStatus("ph-sensor-main", "water.ph", calibrations)).toBe("UNKNOWN");
		});

		it("correctly identifies exact boundary timestamps for pH calibrations (30 days)", () => {
			const now = new Date("2026-08-14T12:00:00Z");
			const exactExpiry = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000).toISOString();
			...
			expect(getSensorCalibrationStatus("ph-sensor-main", "water.ph", calibrations, now)).toBe("VALID");

			// 1ms before expiry -> valid
			const justBefore = new Date(new Date(exactExpiry).getTime() - 1);
			expect(getSensorCalibrationStatus("ph-sensor-main", "water.ph", calibrations, justBefore)).toBe("DUE_SOON");

			// 1ms after expiry -> expired
			const justAfter = new Date(new Date(exactExpiry).getTime() + 1);
			expect(getSensorCalibrationStatus("ph-sensor-main", "water.ph", calibrations, justAfter)).toBe("CALIBRATION_DUE");
		});
		...
		it("returns 'failed' status immediately if latest calibration failed, even if within validity window", () => {
			...
			expect(getSensorCalibrationStatus("ph-sensor-main", "water.ph", [recordFailed], now)).toBe("FAILED");
		});

		it("evaluates EC sensor default 60-day validity window when validUntil is omitted", () => {
			const now = new Date("2026-08-14T12:00:00Z");
			const performed59DaysAgo = new Date(now.getTime() - 59 * 24 * 60 * 60 * 1000).toISOString();
			const performed61DaysAgo = new Date(now.getTime() - 61 * 24 * 60 * 60 * 1000).toISOString();
			...
			expect(getSensorCalibrationStatus("ec-sensor-main", "water.ec", [record59], now)).toBe("DUE_SOON");
			expect(getSensorCalibrationStatus("ec-sensor-main", "water.ec", [record61], now)).toBe("CALIBRATION_DUE");
		});
	});
