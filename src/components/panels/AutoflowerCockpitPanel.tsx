...
export function AutoflowerCockpitPanel({
	...
	// Filter State
	const [activeKindTab, setActiveKindTab] = useState<"all" | CultivarKind>("all");
	const [searchQuery, setSearchQuery] = useState<string>("");
	const [selectedBreeder, setSelectedBreeder] = useState<string>("");
	const [selectedShop, setSelectedShop] = useState<string>("");
	const [selectedTypes, setSelectedTypes] = useState<Set<CultivarType>>(new Set());
	const [selectedProv, setSelectedProv] = useState<Set<PlantProvenance>>(new Set());
	const [selectedLevels, setSelectedLevels] = useState<Set<ExperienceLevel>>(new Set());
	...
	const [drawerStrain, setDrawerStrain] = useState<AutoflowerStrain | null>(null);
	...
	const filteredStrains = useMemo(() => {
		...
		const filtered = cockpitData.filter((strain) => {
			...
			// Level filter
			if (selectedLevels.size > 0 && !selectedLevels.has(strain.level as ExperienceLevel)) {
				return false;
			}
	...
	const kpis = useMemo(() => {
		...
		const originalCount = filteredStrains.filter((s) => s.prov === "original").length;
	...
	return (
		<div className="autoflower-cockpit-root page-stack" style={{ gap: "var(--space-md)" }}>
			...
			>
				<div
					style={{
						...
						inset: "auto 0 0 0",
						height: "2px",
						background: "linear-gradient(90deg, var(--blue), var(--red) 65%, transparent)",
					}}
				/>
				...
				>
					<div>
						...
						>
							Masterclass-Datenbasis aus 61 verifizierten Sorten & Phänotypen für das 60×60×180 cm Zelt.
							Wissenschaftliche DLI-Ertragsprognose, Terpenchemie & Agronomiemodelle.
						</p>
					</div>
					...
					>
						<button
							...
							style={{
								...
								border: viewMode === "cards" ? "1px solid var(--line-strong)" : "none",
								borderRadius: "4px",
								padding: "6px 12px",
								fontSize: "12px",
								fontWeight: 600,
						...
						<button
							...
							style={{
								background: viewMode === "list" ? "var(--surface-0)" : "transparent",
								color: viewMode === "list" ? "var(--green)" : "var(--muted)",
								border: viewMode === "list" ? "1px solid var(--line-strong)" : "none",
								borderRadius: "4px",
								padding: "6px 12px",
								fontSize: "12px",
								fontWeight: 600,
				...
				>
					...
					>
						<div style={{ color: "var(--text)", fontWeight: 600 }}>
							E<sub>gesamt</sub> = 140 W × <span style={{ color: "var(--green)" }}>0,45–0,90 g/W</span> × q
						</div>
						<small
							style={{
								color: "var(--muted)",
						...
						>
							Ertrag ist licht- und flächenlimitiert, nicht pflanzenzahlskaliert.
						</small>
					</div>
					...
					>
						...
						>
							{kpis.total} <span style={{ fontSize: "13px", color: "var(--muted)" }}>/ {cockpitData.length}</span>
						</div>
					</div>
					...
					>
						...
						>
							{kpis.originalCount} <span style={{ fontSize: "12px", color: "var(--muted)" }}>Breeder-Linien</span>
						</div>
					</div>
					...
					>
						...
						>
							{kpis.whiteLabelCount} <span style={{ fontSize: "12px", color: "var(--muted)" }}>Reseller-Lots</span>
						</div>
					</div>
					...
					>
						...
						>
							63–126 g <span style={{ fontSize: "12px", color: "var(--muted)" }}>/ Zelt</span>
						</div>
					</div>
				</div>
			</section>
			...
			>
				...
				>
					...
					<div>
						...
						>
							<button
								...
								style={{
									flex: 1,
									background: activeKindTab === "all" ? "var(--surface-0)" : "transparent",
									color: activeKindTab === "all" ? "var(--text)" : "var(--muted)",
									border: activeKindTab === "all" ? "1px solid var(--line-strong)" : "none",
									borderRadius: "4px",
									padding: "8px 4px",
									fontSize: "11px",
									fontWeight: 700,
							...
							>
								...
								<span
									style={{
										...
										fontSize: "9px",
										fontFamily: "var(--font-mono)",
										color: activeKindTab === "all" ? "var(--green)" : "var(--muted)",
										marginTop: "2px",
									}}
								>
									61
							...
							<button
								...
								style={{
									flex: 1.2,
									background: activeKindTab === "jungpflanze" ? "var(--surface-0)" : "transparent",
									color: activeKindTab === "jungpflanze" ? "var(--text)" : "var(--muted)",
									border: activeKindTab === "jungpflanze" ? "1px solid var(--line-strong)" : "none",
									borderRadius: "4px",
									padding: "8px 4px",
									fontSize: "11px",
									fontWeight: 700,
							...
							>
								...
								<span
									style={{
										...
										fontSize: "9px",
										fontFamily: "var(--font-mono)",
										color: activeKindTab === "jungpflanze" ? "var(--green)" : "var(--muted)",
										marginTop: "2px",
									}}
								>
									Top 50
							...
							<button
								...
								style={{
									flex: 1,
									background: activeKindTab === "samen" ? "var(--surface-0)" : "transparent",
									color: activeKindTab === "samen" ? "var(--text)" : "var(--muted)",
									border: activeKindTab === "samen" ? "1px solid var(--line-strong)" : "none",
									borderRadius: "4px",
									padding: "8px 4px",
									fontSize: "11px",
									fontWeight: 700,
							...
							>
								...
								<span
									style={{
										...
										fontSize: "9px",
										fontFamily: "var(--font-mono)",
										color: activeKindTab === "samen" ? "var(--green)" : "var(--muted)",
										marginTop: "2px",
									}}
								>
									Top 11
							...
							{(
								...
								return (
									<button
										key={typ}
										type="button"
										onClick={() => toggleSetItem(selectedTypes, setSelectedTypes, typ)}
										aria-pressed={active}
										style={{
					...
					<div>
						...
						<div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
							{[
								{ id: "original" as PlantProvenance, label: "Original", color: "var(--green)" },
								{ id: "whitelabel" as PlantProvenance, label: "White Label", color: "var(--amber)" },
								{ id: "unklar" as PlantProvenance, label: "Unklar", color: "var(--red)" },
							].map((p) => {
								...
								return (
									<button
										key={p.id}
										type="button"
										onClick={() => toggleSetItem(selectedProv, setSelectedProv, p.id)}
										aria-pressed={active}
										style={{
											background: active ? "var(--surface-2)" : "transparent",
							...
							{(
								...
								return (
									<button
										key={lvl}
										type="button"
										onClick={() => toggleSetItem(selectedLevels, setSelectedLevels, lvl)}
										aria-pressed={active}
										style={{
											background: active ? "var(--surface-2)" : "transparent",
											border: active ? "1px solid var(--green)" : "1px solid var(--line)",
							...
							{[
								...
								return (
									<button
										key={m.id}
										type="button"
										onClick={() => toggleSetItem(selectedMold, setSelectedMold, m.id)}
										aria-pressed={active}
										style={{
											background: active ? "var(--surface-2)" : "transparent",
							...
							{[
								...
								return (
									<button
										key={f.id}
										type="button"
										onClick={() => toggleSetItem(selectedFeed, setSelectedFeed, f.id)}
										aria-pressed={active}
										style={{
											background: active ? "var(--surface-2)" : "transparent",
				...
				<main style={{ minWidth: 0 }}>
					...
					>
						<div>
							{filteredStrains.length} {filteredStrains.length === 1 ? "Sorte gefunden" : "Sorten gefunden"}
						</div>
					</div>
					...
					{filteredStrains.length === 0 ? (
						...
						>
							...
							<p style={{ margin: "0 0 20px 0", fontSize: "14px", maxWidth: "48ch", marginInline: "auto" }}>
								Die gewählten Kriterien schließen alle Einträge aus. Bitte Höhengrenze anheben, Suchbegriff anpassen oder Filter zurücksetzen.
							</p>
							<button
								type="button"
								onClick={handleResetFilters}
						...
						>
							{filteredStrains.map((strain) => {
								const isSelected = selectedStrainId === strain.id || selectedStrainId === strain.name;
								const provColor =
								...
								return (
									<article
										...
										style={{
											...
											boxShadow: isSelected ? "0 0 16px rgba(103, 214, 174, 0.15)" : "none",
										}}
									>
										{/* Card Header Bar */}
										<div
										...
										>
											<div>
												...
												>
													...
													<span>•</span>
													<span style={{ color: "var(--text-2)" }}>{strain.typ}</span>
													<span>•</span>
													<span style={{ color: strain.kind === "samen" ? "var(--blue)" : "var(--green)" }}>
														{strain.kind === "samen" ? "Saatgut" : "Jungpflanze"}
													</span>
												</div>
											</div>
											...
											>
												...
												>
													<span style={{ color: "var(--muted)", fontSize: "11px" }}>Potenz & Chemotyp:</span>
											...
											>
												<div>
													<span style={{ color: "var(--muted)", display: "block" }}>Dauer:</span>
													<span style={{ color: "var(--text)", fontWeight: 600 }}>{strain.zeit.slice(0, 22)}</span>
												</div>
												<div>
													<span style={{ color: "var(--muted)", display: "block" }}>Höhe:</span>
													<span style={{ color: "var(--text)", fontWeight: 600 }}>
														{strain.hmax ? `${strain.hmin ?? "?"}–${strain.hmax} cm` : strain.hoehe.slice(0, 18)}
													</span>
												</div>
												<div>
													<span style={{ color: "var(--muted)", display: "block" }}>Schimmel:</span>
													<span style={{ color: "var(--text)" }}>{strain.mold.slice(0, 14)}</span>
												</div>
												<div>
													<span style={{ color: "var(--muted)", display: "block" }}>Level:</span>
													<span style={{ color: "var(--text)" }}>{strain.level}</span>
												</div>
											</div>
											...
											>
												<strong>Aroma:</strong> {strain.geschmack || strain.wirkung}
											</div>
										</div>
										...
										>
											...
											{onSelectStrain && (
												<button
													...
													style={{
														flex: 1.2,
														background: isSelected ? "var(--green-dim)" : "var(--green)",
														border: "1px solid var(--green)",
														borderRadius: "var(--radius-sm)",
														padding: "8px 10px",
														color: isSelected ? "var(--green)" : "var(--on-green)",
														fontSize: "12px",
														fontWeight: 700,
														cursor: "pointer",
														textAlign: "center",
												...
												>
													{isSelected ? "✓ Ausgewählt" : isModal ? "In Setup übernehmen" : "Auswählen"}
												</button>
											)}
										</div>
									</article>
						...
						>
							...
							<div
								style={{
									display: "grid",
									gridTemplateColumns: "44px minmax(200px, 1.6fr) minmax(140px, 1fr) 100px 90px",
									gap: "12px",
									padding: "12px 16px",
									background: "var(--surface-1)",
									borderBottom: "1px solid var(--line)",
					...
					>
						...
						>
							<div>
								...
								>
									Rang #{String(drawerStrain.rank).padStart(2, "0")} • {drawerStrain.shop}
								</div>
								<h2
									id="drawer-title"
									style={{
						...
						<div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
							...
							<span
								style={{
									...
									fontWeight: 600,
									background: "var(--surface-2)",
									color: drawerStrain.prov === "original" ? "var(--green)" : "var(--amber)",
									padding: "4px 10px",
									borderRadius: "12px",
									border: "1px solid var(--line)",
								}}
						...
						>
							...
							>
								...
								<span style={{ fontSize: "16px", color: "var(--muted)", fontWeight: 500 }}>trocken</span>
							</div>
							<p style={{ margin: "8px 0 0 0", fontSize: "12px", color: "var(--text-2)", lineHeight: 1.4 }}>
						...
						<div>
							...
							>
								...
								>
									<small style={{ color: "var(--muted)", display: "block", fontSize: "11px" }}>Genetik</small>
									<strong style={{ color: "var(--text)", fontSize: "13px" }}>{drawerStrain.gen}</strong>
								</div>
								...
								>
									<small style={{ color: "var(--muted)", display: "block", fontSize: "11px" }}>Kreuzung</small>
									<strong style={{ color: "var(--text)", fontSize: "13px" }}>{drawerStrain.cross}</strong>
								</div>
								...
								>
									<small style={{ color: "var(--muted)", display: "block", fontSize: "11px" }}>THC-Gehalt</small>
									<div style={{ color: "var(--text)", fontSize: "13px" }}>{drawerStrain.thc}</div>
								</div>
								...
								>
									<small style={{ color: "var(--muted)", display: "block", fontSize: "11px" }}>CBD & CBN</small>
								...
								>
									<small style={{ color: "var(--muted)", display: "block", fontSize: "11px" }}>Minor Cannabinoids</small>
									<div style={{ color: "var(--text-2)", fontSize: "13px" }}>{drawerStrain.minor}</div>
								</div>
							</div>
						</div>
						...
						<div>
							...
							<dl style={{ margin: 0, display: "flex", flexDirection: "column", gap: "10px" }}>
								<div style={{ background: "var(--surface-1)", padding: "12px", borderRadius: "var(--radius-sm)", border: "1px solid var(--line)" }}>
									<dt style={{ fontSize: "11px", color: "var(--muted)", textTransform: "uppercase", marginBottom: "4px" }}>
										Terpenprofil & Quelllage
									</dt>
									<dd style={{ margin: 0, fontSize: "13px", color: "var(--text)" }}>
								...
								<div style={{ background: "var(--surface-1)", padding: "12px", borderRadius: "var(--radius-sm)", border: "1px solid var(--line)" }}>
									...
									<dd style={{ margin: 0, fontSize: "13px", color: "var(--text-2)" }}>{drawerStrain.ester}</dd>
								</div>
						...
						<div>
							...
							<div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
								<div style={{ background: "var(--surface-1)", padding: "12px", borderRadius: "var(--radius-sm)", border: "1px solid var(--line)" }}>
									...
									<div style={{ fontSize: "13px", color: "var(--text-2)" }}>{drawerStrain.feed_note}</div>
								</div>
								...
								<div style={{ background: "var(--surface-1)", padding: "12px", borderRadius: "var(--radius-sm)", border: "1px solid var(--line)" }}>
									...
									<div style={{ fontSize: "13px", color: "var(--text-2)" }}>{drawerStrain.mold_note}</div>
								</div>
								...
								<div style={{ background: "var(--surface-1)", padding: "12px", borderRadius: "var(--radius-sm)", border: "1px solid var(--line)" }}>
									...
									<div style={{ fontSize: "13px", color: "var(--text-2)" }}>{drawerStrain.level_note}</div>
								</div>
								...
								<div style={{ background: "var(--surface-1)", padding: "12px", borderRadius: "var(--radius-sm)", border: "1px solid var(--line)" }}>
									...
									<div style={{ fontSize: "13px", color: "var(--text)" }}>
										<div><strong>Dauer:</strong> {drawerStrain.zeit}</div>
										<div><strong>Höhe:</strong> {drawerStrain.hoehe}</div>
										<div><strong>Lieferform:</strong> {drawerStrain.form}</div>
									</div>
								</div>
							</div>
						</div>
						...
						<div>
							...
							<div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
								<div style={{ background: "var(--surface-1)", padding: "12px", borderRadius: "var(--radius-sm)", border: "1px solid var(--line)" }}>
									...
									<div style={{ fontSize: "13px", color: "var(--text)" }}>{drawerStrain.med}</div>
								...
								<div style={{ background: "var(--surface-1)", padding: "12px", borderRadius: "var(--radius-sm)", border: "1px solid var(--line)" }}>
									<div style={{ fontSize: "11px", color: "var(--green)", textTransform: "uppercase", fontWeight: 700, marginBottom: "4px" }}>
										Masterplan-Projekturteil & Training
									</div>
									<div style={{ fontSize: "13px", color: "var(--text)", fontWeight: 600 }}>{drawerStrain.urteil}</div>
									<div style={{ fontSize: "11px", color: "var(--muted)", marginTop: "4px" }}>
						...
						>
							Modellfelder Schimmelresistenz, Düngerverträglichkeit und Erfahrungsniveau sind Ableitungen [C], keine Messungen.
							THC-Werte ohne Laborhinweis sind Herstellerangaben. Klinische Evidenz besteht für Cannabinoide und Chemotypen, nicht für Sortennamen.
							Bewurzelte Jungpflanzen unterliegen der VG-Köln-Linie (1 L 1051/26 v. 22.06.2026) — Marktanalyse, keine Rechtsfreigabe.
						</footer>
					</div>
				</div>
			)}
