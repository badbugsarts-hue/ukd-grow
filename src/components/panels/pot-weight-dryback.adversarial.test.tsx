...
import type { ExperienceLens, PotProfile, RunPackage } from "../../types";
import {
  DailyOperatorPanel,
  ...
  describe("1. Boundary & Extreme Mathematical Calculations", () => {
    ...
    it("fuzz tests 1,000 random pot configurations ensuring invariants always hold", () => {
      ...
      for (let i = 0; i < 1000; i++) {
        const emptyMass =
          Math.random() < 0.1 ? null : Math.floor(Math.random() * 3000);
        const satMass =
          Math.random() < 0.1 ? null : Math.floor(Math.random() * 10000);
        const nomVol = Math.random() < 0.1 ? 0 : Math.floor(Math.random() * 50);
        const actVol =
          Math.random() < 0.1 ? null : Math.floor(Math.random() * 50);
