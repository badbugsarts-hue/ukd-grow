...
import type { ExperienceLens, PpfdMapPoint, PpfdMapPosition, RunPackage } from "../../types";
		...
		>
			...
			>
				...
				<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
					<h3 id="ppfd-modal-title" style={{ margin: 0, fontSize: "18px", color: "var(--text)" }}>
				...
				<form onSubmit={handleFormSubmit}>
					...
					<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px", marginBottom: "20px" }}>
						<div>
							<label htmlFor="fixture-height-input" style={{ display: "block", fontSize: "12px", color: "var(--text-2)", marginBottom: "4px" }}>
						...
						<div>
							<label htmlFor="dimmer-percent-slider" style={{ display: "block", fontSize: "12px", color: "var(--text-2)", marginBottom: "4px" }}>
						...
						<div>
							<label htmlFor="measurement-device-input" style={{ display: "block", fontSize: "12px", color: "var(--text-2)", marginBottom: "4px" }}>
								Messgerät / Sensor
							</label>
							<input
					...
					<label style={{ display: "block", fontSize: "13px", fontWeight: 700, color: "var(--green)", marginBottom: "8px" }}>
					...
					>
						{DEFAULT_POSITIONS.map(({ pos, label }) => {
							...
							return (
								...
								>
									<span style={{ fontSize: "10px", color: "var(--muted)", display: "block", fontWeight: 700 }}>
										{pos} ({label})
									</span>
									<input
					...
					<div style={{ background: "var(--surface-1)", border: "1px solid var(--line-strong)", borderRadius: "var(--radius-sm)", padding: "16px", marginBottom: "20px" }}>
						<span style={{ fontSize: "11px", color: "var(--green)", fontWeight: 800, textTransform: "uppercase", display: "block", marginBottom: "8px" }}>
							LIVE METRIKEN BERECHNUNG
						</span>
						<div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "10px", textAlign: "center" }}>
							<div>
								<span style={{ fontSize: "11px", color: "var(--muted)" }}>Mittelwert</span>
								<strong style={{ display: "block", fontSize: "16px", color: "var(--text)" }}>{summary.mean} µmol</strong>
							</div>
							<div>
								<span style={{ fontSize: "11px", color: "var(--muted)" }}>Minimum</span>
								<strong style={{ display: "block", fontSize: "16px", color: "var(--text-2)" }}>{summary.min} µmol</strong>
							</div>
							<div>
								<span style={{ fontSize: "11px", color: "var(--muted)" }}>Maximum</span>
								<strong style={{ display: "block", fontSize: "16px", color: "var(--text-2)" }}>{summary.max} µmol</strong>
							</div>
							<div>
								<span style={{ fontSize: "11px", color: "var(--muted)" }}>
									<TermTooltip term="Uniformität" customText="Ziel: > 80% (Min/Mean)" lens={lens}>Uniformität</TermTooltip>
								</span>
								<strong style={{ display: "block", fontSize: "16px", color: summary.uniformity >= 0.8 ? "var(--green)" : "var(--amber)" }}>
