...
export const SensorCalibrationModal: React.FC<SensorCalibrationModalProps> = ({
  ...
  const initialDeviceId =
    run.devices.find((device) =>
      device.capabilities.some(
        (capability) => capability.metric === initialMetric,
      ),
    )?.id ??
    run.calibrations.find((record) => record.metric === initialMetric)
      ?.deviceId ??
    "";
  ...
  const handleMetricChange = (newMetric: MeasurementMetric) => {
    ...
    setDeviceId(
      run.devices.find((device) =>
        device.capabilities.some(
          (capability) => capability.metric === newMetric,
        ),
      )?.id ??
        run.calibrations.find((record) => record.metric === newMetric)
          ?.deviceId ??
        "",
    );
    setBufferStandard("");
      ...
      >
        ...
        {step === 1 && (
          <div>
            ...
            <div style={{ marginBottom: "20px" }}>
              <label
                htmlFor="calibration-reference-standard"
                style={{
                  display: "block",
                  fontSize: "12px",
                  color: "var(--text-2)",
        ...
        {step === 2 && (
          <div>
            ...
            <div style={{ marginBottom: "16px" }}>
              <div>
                <label
                  htmlFor="calibration-uncertainty"
                  style={{
                    display: "block",
                    fontSize: "12px",
                    color: "var(--text-2)",
            ...
            <div style={{ marginBottom: "20px" }}>
              ...
              <div
                role="group"
                aria-labelledby="calibration-result-label"
        ...
        {step === 3 && (
          <div>
            ...
            <div style={{ marginBottom: "16px" }}>
              <label
                htmlFor="calibration-validity-days"
                style={{
                  display: "block",
                  fontSize: "12px",
                  color: "var(--text-2)",
