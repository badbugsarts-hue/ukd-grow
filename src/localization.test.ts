...
import { displayMeasurement, formatLocalDateTime, toCanonicalSi } from "./localization";
...
describe("localization and canonical SI storage", () => {
	...
	it("uses regional decimal formats", () => {
		expect(displayMeasurement(25.5, "temperature", "si", "de-DE")).toBe("25,5 °C");
		expect(displayMeasurement(25.5, "temperature", "si", "en-US")).toBe("25.5 °C");
	});
	it("formats UTC with an explicit timezone", () => {
		expect(formatLocalDateTime("2026-08-18T12:00:00Z", "en-US", "UTC")).toContain("12:00");
	});
});
