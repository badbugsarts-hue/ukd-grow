...
test("Live-Preflight, Today, AI exchange, proposal decision and backup work end-to-end", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Simulation als Live-Run starten" }),
  ).toBeVisible();
  await page
    .getByRole("button", { name: /(Aussaat|Durchstoß) bestätigen/ })
    .click();
  await expect(mode).toContainText("LIVE", { timeout: 30_000 });
  await expect(mode).toContainText("Tag 0");
  await expect(page.locator(".live-status-strip")).toBeVisible();
  await expect(page.locator(".live-status-strip")).toContainText(
    /\d{1,2}\.\d{1,2}\.\d{4}/,
  );
    ...
    proposals: [
      {
        ...
        baseValue: null,
        proposedValue:
          "Wasser-Baseline vor einer chemieabhängigen Entscheidung messen.",
        reason:
          "Die Wasserchemie ist unvollständig; zuerst messen statt einen Wert zu erfinden.",
        uncertainty: "low",
  ...
  await expect(
    page.getByRole("heading", { name: "AI-Vorschläge" }),
  ).toBeVisible();
  ...
  await page
    ...
    .getByRole("button", { name: "Sofortbackup", exact: true })
    .click();
  const backup = await backupDownload;
  expect(backup.suggestedFilename()).toContain(".ukdbackup");
});

test("global Quick Log records measured values with audit and timeline", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText(/Quick Log wurde mit Audit- und Timeline-Spur gespeichert/),
  ).toBeVisible();
...
test("shared dialogs trap focus, close with Escape and restore the trigger", async ({
  page,
}) => {
