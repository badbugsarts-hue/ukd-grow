...
const cultivars = JSON.parse(fs.readFileSync("./src/data/autoflower-cockpit.json", "utf8"));
...
cultivars.forEach((c, idx) => {
  ...
  const required = [
    "rank", "name", "shop", "score", "id", "breeder", "prov", "warn", "form", "gen", 
    "indica", "sativa", "cross", "thc", "cbd", "cbn", "minor", "ester", "wirkung", 
    "geschmack", "geruch", "terpene_src", "terpene", "reviews", "med", "med_src", 
    "feed", "feed_note", "mold", "mold_note", "level", "level_note", "zeit", "hoehe", 
    "hmin", "hmax", "ertrag_lo", "ertrag_hi", "ertrag_src", "urteil", "evidenz", "q", 
    "kind", "typ"
  ];
...
console.log("Breeders represented (" + new Set(cultivars.map(c => c.breeder)).size + "):", [...new Set(cultivars.map(c => c.breeder))].sort());
