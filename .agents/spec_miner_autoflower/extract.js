...
const html = fs.readFileSync(path.join(__dirname, '../../plan/Autoflower-Cockpit-v3.html'), 'utf8');
...
console.log('\n=== JUNGPFLANZEN LIST ===');
jp.forEach(d => {
  console.log(`[Rank ${d.rank}] ${d.name} (${d.breeder}) | Score: ${d.score} | Prov: ${d.prov} | Yield: ${d.ertrag_lo}-${d.ertrag_hi}g (q=${d.q}) | Height: ${d.hmin ?? '?'}-${d.hmax ?? '?'}cm | Level: ${d.level} | Mold: ${d.mold} | Feed: ${d.feed}`);
});

console.log('\n=== SAATGUT LIST ===');
sm.forEach(d => {
  console.log(`[Rank ${d.rank}] ${d.name} (${d.breeder}) | Type: ${d.typ} | Score: ${d.score} | Prov: ${d.prov} | Yield: ${d.ertrag_lo}-${d.ertrag_hi}g (q=${d.q}) | Height: ${d.hmin ?? '?'}-${d.hmax ?? '?'}cm | Level: ${d.level} | Mold: ${d.mold} | Feed: ${d.feed}`);
});
...
fs.writeFileSync(path.join(__dirname, 'extracted_plant_data.json'), JSON.stringify(DATA, null, 2));
